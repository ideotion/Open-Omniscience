...
#   OO_USE_LOCK=0                  skip requirements.lock, always floor-resolve live
#                                   (the lock is used automatically when present,
#                                   matches this Python, and covers the requested
#                                   components -- see docs/CONTRIBUTING.md)
#
...
# requirements.lock is generated (docs/CONTRIBUTING.md) by `pip-compile --extra=X ...
# --output-file=requirements.lock`, so its own header is the one place that says which
# extras it actually covers -- never hand-maintained, so it can't drift silently out of
# sync with a lock regen. Echoes the covered extras, one per line.
_lock_covered_extras() {
    local lock="$1"
    grep -m1 -- '--output-file=requirements.lock' "$lock" 2>/dev/null \
        | grep -oE -- '--extra[= ][A-Za-z0-9_.-]+' | sed -E 's/--extra[= ]//'
}

# True only when every requested extra is among the ones requirements.lock was
# generated with. A lock covering fewer extras than were asked for must not be used
# for the ones it doesn't have -- that would silently install unpinned versions for
# them while claiming the audited path.
_lock_covers_extras() {
    local extras="$1" lock="$2" w c ok
    local -a covered=() want=()
    while IFS= read -r c; do [ -n "$c" ] && covered+=("$c"); done < <(_lock_covered_extras "$lock")
    IFS=',' read -ra want <<<"$extras"
    for w in ${want[@]+"${want[@]}"}; do
        [ -n "$w" ] || continue
        ok=0
        for c in ${covered[@]+"${covered[@]}"}; do [ "$w" = "$c" ] && ok=1 && break; done
        [ "$ok" = 1 ] || return 1
    done
    return 0
}

# requirements.lock pins exact wheel hashes, which are Python-ABI-specific
# (cp313 != cp314). True only when the running interpreter matches the minor
# version the lock's own "autogenerated by pip-compile with Python X.Y" header
# records -- a mismatch would just fail loudly on --require-hashes, so we skip
# straight to the safe (floor-based) path instead of trying and failing.
_lock_matches_python() {
    local lock="$1" want got
    want="$(grep -m1 -oE 'pip-compile with Python [0-9]+\.[0-9]+' "$lock" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+' || true)"
    [ -n "$want" ] || return 1
    got="$(python -c 'import sys; print("%d.%d" % sys.version_info[:2])')"
    [ "$want" = "$got" ]
}

# The reproducible/release install path docs/CONTRIBUTING.md documents:
# hash-verified, CVE-audited pins from requirements.lock, instead of a live
# floor-based resolve against whatever PyPI serves that day. Declines (returns 1,
# never dies) rather than using the lock when it can't honestly serve this
# install -- pip_install() falls back to the existing floor-based path with a
# loud explanation in every such case.
_pip_install_from_lock() {
    local extras="$1" pip_opts="$2" pip_tmp="$3" lock="$SRC_DIR/requirements.lock"
    [ -f "$lock" ] || return 1
    if ! _lock_matches_python "$lock"; then
        warn "requirements.lock was generated for a different Python minor version than $(python -V 2>&1) -- using the floor-based (pyproject.toml) install instead."
        return 1
    fi
    if ! _lock_covers_extras "$extras" "$lock"; then
        warn "requirements.lock does not cover the requested component(s) '${extras:-core}' -- using the floor-based (pyproject.toml) install instead. (Regenerate the lock with the extras you need: see docs/CONTRIBUTING.md.)"
        return 1
    fi
    step "Installing the audited, hash-verified pin set (requirements.lock)"
    local log; log="$(mktemp "${pip_tmp}/pip-lock-log-XXXXXX" 2>/dev/null || echo "${pip_tmp}/pip-lock.log")"
    if ! TMPDIR="$pip_tmp" python -m pip install $pip_opts --require-hashes -r "$lock" 2>&1 | tee "$log"; then
        rm -f "$log"
        warn "Could not install the hash-verified pin set from requirements.lock (a wheel may be unavailable for this platform, or the pins may need a refresh) -- falling back to the floor-based install."
        return 1
    fi
    # requirements.lock pins THIRD-PARTY dependencies only; it does not (cannot) also
    # install the project package itself. --require-hashes refuses a local/editable
    # requirement outright ("cannot verify hashes ... no single file to hash"), so
    # this has to be a second, unguarded call -- --no-deps keeps it from re-resolving
    # (and potentially drifting away from) the pinned set that was just verified above.
    if ! TMPDIR="$pip_tmp" python -m pip install $pip_opts -e "." --no-deps 2>&1 | tee "$log"; then
        rm -f "$log"
        warn "Installed the hash-verified pin set, but could not install the project package itself from source -- falling back to the floor-based install."
        return 1
    fi
    rm -f "$log"
    ok "Python packages installed (audited pins: requirements.lock)"
    return 0
}
...
pip_install() {
    ...
    # The reproducible/release install path (docs/CONTRIBUTING.md): try
    # requirements.lock first, when it's present and actually covers this install.
    # OO_USE_LOCK=0 opts back into the plain floor-based resolve unconditionally.
    if [ "${OO_USE_LOCK:-1}" != "0" ] && _pip_install_from_lock "$extras" "$pip_opts" "$pip_tmp"; then
        return 0
    fi
    step "Installing the app: $spec"
