...
Examples:
  ...
  python scripts/setup_llm.py --model llama3:8b --model mistral:7b  # Download multiple models
    ...
    parser.add_argument(
        "--install-ollama",
        action="store_true",
        help="Install Ollama if not already installed"
    )
    parser.add_argument(
        "--download-models",
        action="store_true",
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Install Ollama and download all default models"
    )
    parser.add_argument(
        "--start",
        action="store_true",
        help="Start Ollama server after installation"
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show current LLM installation status"
    )
    ...
    if args.status or (not any([args.install_ollama, args.download_models, args.model, args.all, args.start])):
        setup.print_status()
