...
def update_file_content(filepath):
    ...
    if 'GNU General Public License' not in content:
        ...
        if content.lstrip().startswith('#!'):
            ...
            lines = content.split('\n')
            lines.insert(1, '')
            lines.insert(2, '# Open Omniscience - Global Intelligence Platform for Investigative Journalism')
            lines.insert(3, '#')
            lines.insert(4, '# Copyright (C) 2026 Ideotion')
            lines.insert(5, '#')
            lines.insert(6, '# This program is free software: you can redistribute it and/or modify')
            lines.insert(7, '# it under the terms of the GNU General Public License as published by')
            ...
            lines.insert(17, '# along with this program.  If not, see <http://www.gnu.org/licenses/>.')
            lines.insert(18, '#')
