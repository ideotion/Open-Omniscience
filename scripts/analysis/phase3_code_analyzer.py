...
class CodeAnalyzer(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            ...
            self.function_calls.append({
                'name': f"{self.get_call_path(node.func)}.{attr_name}",
                'line': node.lineno
            })
    ...
    def check_security_issue(self, name, line):
        ...
        security_checks = {
            'eval': ('EVAL_USAGE', 'CRITICAL', 'Use of eval() is dangerous and can lead to code injection'),
            'exec': ('EXEC_USAGE', 'CRITICAL', 'Use of exec() is dangerous and can lead to code injection'),
            'pickle.load': ('UNSAFE_DESERIALIZATION', 'HIGH', 'pickle.load() can lead to arbitrary code execution'),
            'pickle.loads': ('UNSAFE_DESERIALIZATION', 'HIGH', 'pickle.loads() can lead to arbitrary code execution'),
            'subprocess.run': ('SHELL_INJECTION_RISK', 'MEDIUM', 'subprocess.run() with shell=True can lead to shell injection'),
            'subprocess.call': ('SHELL_INJECTION_RISK', 'MEDIUM', 'subprocess.call() with shell=True can lead to shell injection'),
            'subprocess.Popen': ('SHELL_INJECTION_RISK', 'MEDIUM', 'subprocess.Popen() with shell=True can lead to shell injection'),
            'os.system': ('SHELL_INJECTION_RISK', 'MEDIUM', 'os.system() can lead to shell injection'),
            'yaml.load': ('UNSAFE_DESERIALIZATION', 'HIGH', 'yaml.load() can lead to arbitrary code execution'),
    ...
    def is_standard_name(self, name):
        ...
        standard_names = {
            'i', 'j', 'k', 'n', 'm', 'x', 'y', 'z',  # Common loop variables
            'self', 'cls', 'super',  # OOP special names
            'args', 'kwargs',  # Function parameters
            'e', 'ex', 'exception', 'error',  # Exception names
            'f', 'file', 'fp',  # File objects
            'key', 'value', 'item',  # Dictionary iteration
            'line', 'lines',  # Line processing
            'data', 'result', 'results',  # Common data names
    ...
    def visit_ExceptHandler(self, node):
        ...
        elif (isinstance(node.body, list) and len(node.body) == 1 and 
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                result['functions'].append({
                    ...
                    'has_type_hints': node.returns is not None or any(arg.annotation is not None for arg in node.args.args)
                })
...
def main():
    ...
    total_functions = sum(f.get('metrics', {}).get('functions', 0) for f in report['files'].values())
    total_classes = sum(f.get('metrics', {}).get('classes', 0) for f in report['files'].values())
    files_with_type_hints = sum(1 for f in report['files'].values() if f.get('metrics', {}).get('has_type_hints'))
    files_with_docstrings = sum(1 for f in report['files'].values() if f.get('metrics', {}).get('has_docstrings'))
