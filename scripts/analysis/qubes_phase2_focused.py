...
class FocusedDependencyChecker:
    ...
    def verify_import(self, ref: dict[str, Any]) -> dict[str, Any]:
        ...
        result = {"reference": ref, "verified": False, "message": "", "severity": "LOW"}
    ...
    def _verify_module_import(
        self, module: str, ref: dict[str, Any], result: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _verify_from_import(
        self, module: str, symbol: str, level: int, ref: dict[str, Any], result: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _verify_relative_import(
        self, module: str, ref: dict[str, Any], result: dict[str, Any], symbol: str = None
    ) -> dict[str, Any]:
        ...
        if module:
            ...
            for part in parts:
                ...
                # Then check for directory (package)
                    ...
                    result["message"] = (
                        f"Relative import not found: {module} from {file} (level={level})"
                    )
                    result["severity"] = "CRITICAL"
                    return result
    ...
    def run(self) -> dict[str, Any]:
        ...
        if self.issues:
            print(f"{'=' * 80}")
            print(
                f"CRITICAL ISSUES FOUND: {len([i for i in self.issues if i.get('severity') == 'CRITICAL'])}"
            )
            print(f"{'=' * 80}\n")
