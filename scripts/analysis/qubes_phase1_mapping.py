...
class FileMapper:
    ...
    def run(self) -> dict[str, Any]:
        ...
        self.file_map["stats"]["total_size_human"] = self.format_size(
            self.file_map["stats"]["total_size_bytes"]
        )
        ...
        print(
            f"Largest File: {self.file_map['stats']['largest_file']['path']} ({self.format_size(self.file_map['stats']['largest_file']['size'])})"
        )
        print(
            f"Smallest File: {self.file_map['stats']['smallest_file']['path']} ({self.format_size(self.file_map['stats']['smallest_file']['size'])})"
        )
        ...
        for file_type, count in sorted(
            self.file_map["stats"]["file_types"].items(), key=lambda x: x[1], reverse=True
        ):
...
def main():
    ...
    parser.add_argument(
        "--root", type=str, default=".", help="Root directory to map (default: current directory)"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        "--exclude", type=str, nargs="*", default=[], help="Additional patterns to exclude"
    )
