...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('import_eml.log')
    ]
)
...
def parse_eml_file(file_path: Path) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        is_newsletter = False
        if subject:
            newsletter_keywords = ['newsletter', 'update', 'digest', 'bulletin', 'brief', 'weekly', 'daily', 'monthly']
...
def extract_source_name(from_address: str, subject: str) -> str:
    ...
    if '@' in email_part:
        ...
        common_providers = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 'aol.com', 'protonmail.com']
...
def create_article_from_email(session, email_data: Dict[str, Any], source: Any, **kwargs) -> Optional[Any]:
    ...
    try:
        ...
        # Check for duplicate
        existing_article = session.query(Article).filter_by(content_hash=article_data['content_hash']).first()
        if existing_article:
            logger.info(f"Duplicate article detected (hash: {article_data['content_hash']}), skipping")
...
Examples:
  ...
  python scripts/import_eml.py /path/to/eml/ --source "Tech News" --category "Technology" --priority 5
    ...
    parser.add_argument(
        '--dry-run', '-n',
        action='store_true',
        help='Test import without saving to database'
    )
