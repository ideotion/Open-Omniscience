...
def _client(tmp_path):
    ...
    engine = create_engine(f"sqlite:///{tmp_path / 'sg.db'}", future=True,
...
def _seed_keyword(s, term, normalized, *, article_ids):
    k = Keyword(term=term, normalized_term=normalized, language="en", frequency=0,
                is_entity=True, entity_type="org")
    s.add(k)
    s.flush()
...
def test_supergroup_crud_and_aggregate(tmp_path):
    ...
    try:
        with TestClient(app) as c:
            ...
            assert c.post("/api/insights/supergroups",
                          json={"name": "Russia–Ukraine war"}).status_code == 409  # duplicate

            # Add members (idempotent).
            r = c.post(f"/api/insights/supergroups/{sid}/members",
                       json={"normalized": ["russia", "ukraine", "russia"]}).json()
            ...
            # Remove a member.
            c.request("DELETE", f"/api/insights/supergroups/{sid}/members",
