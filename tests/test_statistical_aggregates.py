...
@pytest.mark.parametrize("code", ["ZZ", "QQQ", "4E", "", "  ", None, "not-a-code"])
def test_unrecognised_codes_are_unknown_not_countries(code):
    ...
    invents a classification. `4E` is a real World Bank aggregate deliberately absent
    from the table (its alpha-2 could not be verified offline) — it must read as unknown,
    not be guessed at.
    """
