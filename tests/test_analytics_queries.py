...
def _mk(db, h, text, when, country="fr"):
    a = Article(url=f"https://x.test/{h}", canonical_url=f"https://x.test/{h}", source_id=1,
                title="T", content=text, hash=h, country=country, language="en",
...
def test_trend_and_top(db):
    _mk(db, "a1", "Inflation rose. Inflation worries grew as inflation spread across markets here.", "2024-01-03")
    _mk(db, "a2", "Inflation again dominated headlines while inflation pressures intensified sharply.", "2024-01-10")
...
def test_associations_pmi_positive_for_cooccurring(db):
    ...
    for i, when in enumerate(["2024-02-01", "2024-02-02", "2024-02-03"]):
        _mk(db, f"c{i}", "New sanctions hit supply. Sanctions affected nickel and nickel exports broadly.", when)
    _mk(db, "d0", "Unrelated weather report about rainfall and rainfall patterns over the coast today.", "2024-02-04")
...
def test_context_snippet_contains_term(db):
    _mk(db, "x1", "The committee discussed wildfire response and wildfire funding for the affected region.", "2024-03-01")
...
def test_map_data_groups_by_area(db):
    _mk(db, "m1", "Election campaigns and election rallies filled the capital this election season now.", "2024-03-05", country="fr")
    _mk(db, "m2", "Drought worsened. Drought and drought relief efforts dominated the regional agenda.", "2024-03-06", country="ke")
...
def test_insights_api_status_and_reindex(tmp_path):
    ...
    engine = create_engine(f"sqlite:///{tmp_path / 'ins.db'}", future=True,
                           connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    ...
    with Sess() as s:
        s.add(Source(name="S", domain="x.test", country="fr"))
        s.commit()
        s.add(Article(url="https://x.test/p", canonical_url="https://x.test/p", source_id=1,
                      title="T", content="Diplomacy and diplomacy talks shaped the summit agenda this week here.",
                      hash="hh", country="fr", language="en",
