...
def test_article_view_shows_co_citation(tmp_path):
    ...
    engine = create_engine(f"sqlite:///{tmp_path / 'cc.db'}", future=True,
                           connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    Sess = sessionmaker(bind=engine, future=True)
    ...
    with Sess() as s:
        ...
        s.commit()
        s.add_all([
            Article(url="https://wire.test/a", canonical_url="https://wire.test/a", source_id=1,
                    title="Article A", content="Body A.", hash="ha", language="en",
                    created_at=datetime.now(UTC)),
            Article(url="https://wire.test/b", canonical_url="https://wire.test/b", source_id=1,
                    title="Article B", content="Body B.", hash="hb", language="en",
                    created_at=datetime.now(UTC)),
        ])
        s.commit()
        # Both articles cite the same external URL (the co-citation signal).
        s.add_all([
            ArticleLink(article_id=1, url=shared, normalized_url=shared,
                        link_text="the report", link_type="external"),
            ArticleLink(article_id=2, url=shared, normalized_url=shared,
                        link_text="the report", link_type="external"),
        ])
        s.commit()
