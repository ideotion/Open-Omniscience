...
def test_reopening_the_import_dialog_offers_the_last_run_without_re_rendering_it():
    """The 2026-07-16 field report's need, under the 2026-09-15 ruling that changed
    the SHAPE of the answer.

    THE NEED (unchanged): a large restore runs for hours as a background job, so the
    tab is very likely closed or reloaded before it finishes -- and openUnifiedImport()
    used to blank #ux-imp-summary on every reopen, discarding the result forever. The
    old fix recovered each job manager's last completed summary and re-rendered the
    WHOLE report on reopen (`_uxShowLastCompletedSummary`).

    THE RULING (R1, Q201 = a): reopening shows a FRESH page, and ONE quiet line
    linking the persisted import report. That is not a weaker answer to the same
    need -- the persisted report survives the tab, the process and the machine, where
    a process-wide singleton's last summary survived only until the next job started.

    So this guard checks what the SURVIVOR does, which is what licenses the removal:
    the line exists, it is wired into the opener, it reads the reports endpoint, and
    it LINKS the report rather than re-rendering it. Weakening it to "the feature is
    gone" would authorise quietly dropping the capability next time."""
    from tests.js_source_helper import function_body, strip_comments

    assert "_uxShowLastCompletedSummary" not in strip_comments(_APP), (
        "the re-rendered last-run summary is retired by R1; a reader must not find it back"
    )
    ...
    line = function_body(_APP, "_uxImLastLineHtml")
    assert "import-reports/${encodeURIComponent(rep.filename)}" in line
    assert "_renderImportSummary" not in line
    # openUnifiedImport() must actually call it (wiring, not just a dangling helper),
    # and must blank the line first so a stale one cannot survive a reopen.
    # strip_comments FIRST: a mutation that commented the call out (`// _uxImLastLine();`)
    # left the needle in the raw source and passed this assertion unchanged (measured
    # 2026-09-16 -- one of two survivors in the S04-02 mutation matrix). A substring test
    # against un-stripped source cannot tell a call from a note about a call.
    opener = strip_comments(function_body(_APP, "openUnifiedImport"))
    assert "_uxImLastLine()" in opener
    assert 'document.getElementById("ux-imp-last").innerHTML = ""' in opener
