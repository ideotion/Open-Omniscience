...
def test_the_pqc_extra_still_excludes_pqcrypto_1_0():
    """The ``[pqc]`` ceiling is deliberate, and prose has now failed to hold it twice.

    ``pqcrypto`` 1.0.0 renames ``generate_keypair`` to ``keygen`` AND inverts ``verify``'s
    contract: 0.4.0 returns True/False, 1.0.0 returns ``None`` for a VALID signature and
    raises for an invalid one — so ``signing.py``'s ``bool(_mldsa.verify(...))`` reports
    every genuine ML-DSA signature as a verification FAILURE on any install whose keys
    already exist. That path never reaches ``generate_keypair``, so it fails SILENTLY, in
    the tamper-evidence chain. Measured with this suite: 1.0.0 → 16 failed / 23 passed;
    0.4.0 → 39 passed. The "PQC signing path" CI lane installs the extra, so a widening
    reddens there and nowhere else — it skips in every other lane and on any dev box
    without it.

    The reason was already written into the constraint comment, in capitals, naming the
    migration doc it requires — and dependabot widened the bound to ``<2.0`` anyway, twice
    (#996 on 2026-09-03, #1012 on 2026-09-07), because the next reader was not a reader.
    Prose addresses humans; only a CI-visible mechanism addresses a bot. This is that
    mechanism, and it is the thing that was missing both times.

    It asserts the PROPERTY rather than the string, so a reformat or a raised floor passes
    and only a real widening reddens. The second assertion is the negative-space twin: a
    ceiling low enough to exclude 1.0.0 but also the version we actually ship would satisfy
    the first assertion while breaking every install.

    Widening this is a MIGRATION, not a version bump — it changes call sites and a return
    contract in a path that touches persisted key material, so it goes through
    docs/maintenance/EXTERNAL_DEPENDENCIES.md. When that lands, update this test
    deliberately; never by reflex to make a red lane green.
    """
    from packaging.requirements import Requirement
    from packaging.version import Version

    pyproject = (_SRC.parent / "pyproject.toml").read_text(encoding="utf-8")
    reqs = [Requirement(m.group(1)) for m in re.finditer(r'"(pqcrypto[^"]*)"', pyproject)]
    assert reqs, "pyproject no longer declares pqcrypto — update this guard deliberately"

    for req in reqs:
        assert not req.specifier.contains(Version("1.0.0")), (
            f"pyproject declares {str(req)!r}, which ADMITS pqcrypto 1.0.0. That release "
            "renames generate_keypair->keygen and inverts verify()'s contract, so "
            "bool(_mldsa.verify(...)) reports every VALID ML-DSA signature as a failure — "
            "silently, in the tamper-evidence path. Keep the <1.0 ceiling; the upgrade is a "
            "migration through docs/maintenance/EXTERNAL_DEPENDENCIES.md, not a bound widening."
        )
        assert req.specifier.contains(Version("0.4.0")), (
            f"pyproject declares {str(req)!r}, which excludes pqcrypto 0.4.0 — the version "
            "this repo's suite is green against. A ceiling that also excludes what we ship "
            "is not a fix."
        )
