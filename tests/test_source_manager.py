...
def sample_sources(db_session):
    ...
    sources = [
        Source(name="BBC News", domain="bbc.com", rss_url="http://feeds.bbci.co.uk/news/rss.xml", 
               rate_limit_ms=1000, enabled=True, priority=1, tags="news,uk"),
        Source(name="Reuters", domain="reuters.com", rss_url="https://www.reuters.com/tools/rss", 
               rate_limit_ms=1000, enabled=True, priority=1, tags="news,financial"),
        Source(name="TechCrunch", domain="techcrunch.com", rss_url="https://techcrunch.com/feed/", 
               rate_limit_ms=2000, enabled=False, priority=2, tags="technology,startups"),
        Source(name="The Guardian", domain="theguardian.com", rss_url="https://www.theguardian.com/world/rss", 
               rate_limit_ms=1500, enabled=True, priority=2, tags="news,uk,opinion"),
    ]
...
def sample_groups(db_session):
    ...
    groups = [
        SourceGroup(name="News Sources", description="Major news organizations", 
                    color="#FF5733", is_tag_based=False, priority=1, rate_limit_ms=1000, enabled=True),
        SourceGroup(name="Tech Sources", description="Technology news sources", 
                    color="#33FF57", is_tag_based=True, tag_pattern="technology,tech", priority=2, rate_limit_ms=2000, enabled=True),
        SourceGroup(name="Disabled Group", description="Disabled sources group", 
                    color="#5733FF", is_tag_based=False, priority=3, rate_limit_ms=3000, enabled=False),
    ]
...
class TestSourceOperations:
    ...
    def test_create_duplicate_source(self, source_manager, sample_sources):
        ...
        new_source = source_manager.create_source(
            name="BBC News Duplicate",
            domain="bbc.com",
            rss_url="http://different-rss.com"
        )
...
class TestBatchSourceOperations:
    ...
    def test_set_source_priority(self, source_manager, sample_sources):
        ...
        count = source_manager.set_source_priority(
            [sample_sources[0].id, sample_sources[1].id],
            priority=3
        )
        assert count == 2
    ...
    def test_remove_tags_from_sources(self, source_manager, sample_sources):
        ...
        # First add some tags
        source_manager.add_tags_to_sources(
            [sample_sources[0].id],
            tags=["premium"]
        )
        
        # Now remove them
        count = source_manager.remove_tags_from_sources(
            [sample_sources[0].id],
            tags=["premium"]
        )
        assert count == 1
...
class TestGroupSourceAssociation:
    ...
    def test_add_source_to_groups(self, source_manager, sample_sources, sample_groups):
        ...
        count = source_manager.add_source_to_groups(
            sample_sources[0].id,
            [sample_groups[0].id, sample_groups[1].id]
        )
...
class TestTagBasedGroups:
    ...
    def test_create_tag_based_group(self, source_manager, sample_sources):
        ...
        group = source_manager.create_tag_based_group(
            name="News Tag Group",
            tag_pattern="news,uk"
        )
    ...
    def test_refresh_tag_based_groups(self, source_manager, sample_sources):
        ...
        source_manager.create_tag_based_group(
            name="Tech Tag Group",
            tag_pattern="technology"
        )
...
class TestMetadataOperations:
    ...
    def test_get_metadata(self, source_manager, sample_sources):
        ...
        source_manager.create_metadata(
            sample_sources[0].id,
            language="en",
            country="GB"
        )
...
class TestBatchGroupOperations:
    ...
    def test_enable_groups(self, source_manager, sample_sources, sample_groups):
        ...
        source_manager.add_sources_to_group(sample_groups[0].id, [sample_sources[0].id, sample_sources[1].id])
