...
_WIRE = (
    "According to Reuters, the central bank raised interest rates by half a point today "
...
def corpus(monkeypatch, tmp_path):
    ...
    for i in range(12):  # 12 FR outlets echoing one wire story
        ...
        s.add(
            Article(
                url=f"https://o{i:02d}.fr/x",
                canonical_url=f"https://o{i:02d}.fr/x",
                source_id=src.id,
                title="Rates decision",
                content=_WIRE,
                hash=f"h{i}",
                language="fr",
                published_at=now - timedelta(hours=20 - i),
                created_at=now,
            )
        )
    s.commit()
