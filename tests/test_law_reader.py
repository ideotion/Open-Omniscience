...
def test_law_document_reader_renders_text_and_diff(tmp_path):
    ...
    engine = create_engine(f"sqlite:///{tmp_path / 'law.db'}", future=True,
                           connect_args={"check_same_thread": False})
    Base.metadata.create_all(engine)
    Sess = sessionmaker(bind=engine, future=True)
    with Sess() as s:
        s.add(LawDocument(
            jurisdiction="uk", title="Data Protection Act", url="https://legislation.gov.uk/dpa",
            official_url="https://legislation.gov.uk/dpa", category="legislation",
            consolidated=True, baseline_text="Section 1.\nEveryone has rights.",
            last_status="ok", last_checked_at=datetime.now(UTC)))
        s.commit()
        s.add(LawRevision(
            document_id=1, content_hash="abc", observed_at=datetime(2026, 1, 2, tzinfo=UTC),
            delta_bytes=42, diff="-old clause removed\n+new clause added", flagged=True,
