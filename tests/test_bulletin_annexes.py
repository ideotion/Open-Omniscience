...
def _row(aid: int, title: str, lang: str = "fr") -> dict:
    return {
        "id": aid,
        "title": title,
        ...
        "source": {"name": "Le Monde", "domain": "lemonde.fr", "country": "fr",
                   "source_type": "news"},
        "asserted": {"published_at": "2026-08-05T09:30:00", "author": "A. Dupont",
                     "language": lang},
        "deduced": {"collected_at": "2026-08-05T10:00:00", "detected_language": lang,
...
def test_the_names_are_the_ones_that_were_asked_for():
    ed = _edition()
    assert bundle_stem(ed) == "20260810_OOS_Bulletin_Weekly"
    assert report_filename(ed) == "20260810_OOS_Bulletin_Weekly.md"
    assert annexes_filename(ed) == "20260810_OOS_Bulletin_Weekly_Annexes.zip"
    assert article_filename(date(2026, 8, 10), "0001") == "20260810_Article_0001.md"


def test_a_repeat_for_the_same_period_gets_a_number_on_every_name():
    ed = _edition()
    assert bundle_stem(ed, ordinal=2) == "20260810_OOS_Bulletin_Weekly_2"
    assert annexes_filename(ed, ordinal=2) == "20260810_OOS_Bulletin_Weekly_2_Annexes.zip"
    assert report_filename(ed, ordinal=3) == "20260810_OOS_Bulletin_Weekly_3.md"
...
def test_the_date_is_the_period_covered_not_the_day_it_was_built():
    """A weekly kept for a year sorts by the week it covers, and the report's name can
    be paired with the record it came from. A generation date would name two different
    weeks identically if both were produced on one afternoon."""
    ed = _edition(period={**_PERIOD, "last_day": "2026-07-05"}, generated_at="2026-08-11T09:00:00")
    assert bundle_stem(ed).startswith("20260705_")


def test_an_exclusive_end_with_no_last_day_still_names_the_right_day():
    ed = _edition(period={"cadence": "weekly", "start": "2026-08-04", "end": "2026-08-11"})
    assert bundle_stem(ed) == "20260810_OOS_Bulletin_Weekly", "end is exclusive"


def test_a_record_with_no_period_refuses_to_be_named():
    """Named after today, a bundle about last month is a filename that lies — and an
    operator keeping a year of them could not notice."""
    with pytest.raises(UnnameableEdition):
        bundle_stem({"period": {}})
    with pytest.raises(UnnameableEdition):
        bundle_stem({"period": {"cadence": "weekly", "end": "not-a-date"}})


def test_the_ordinal_is_a_position_so_a_re_download_keeps_its_name():
    """A next-free counter would rename the same edition on every download, which is
    the opposite of what a filename is for."""
    siblings = [
        {"filename": "20260810-OOS-weekly-aaaaaaaa.json", "covers_through": "2026-08-10",
         "cadence": "weekly"},
        {"filename": "20260810-OOS-weekly-bbbbbbbb.json", "covers_through": "2026-08-10",
         "cadence": "weekly"},
        {"filename": "20260803-OOS-weekly-cccccccc.json", "covers_through": "2026-08-03",
         "cadence": "weekly"},
    ]
    ...
    assert edition_ordinal("20260810-OOS-weekly-bbbbbbbb.json", siblings) == 2
    # A different period is not a repeat.
...
def test_a_monthly_and_a_weekly_ending_on_one_day_are_not_repeats_of_each_other():
    siblings = [
        {"filename": "20260810-OOS-weekly-aaaaaaaa.json", "covers_through": "2026-08-10",
         "cadence": "weekly"},
        {"filename": "20260810-OOS-monthly-bbbbbbbb.json", "covers_through": "2026-08-10",
         "cadence": "monthly"},
    ]
...
def test_the_bundle_is_one_file_per_cited_article_plus_a_contents_page(corpus):
    ...
    assert out["filename"] == "20260810_OOS_Bulletin_Weekly_Annexes.zip"
    assert out["articles"] == 3
    assert names == [
        "20260810_OOS_Bulletin_Weekly/00_Table_of_Contents.md",
        "20260810_OOS_Bulletin_Weekly/20260810_Article_0001.md",
        "20260810_OOS_Bulletin_Weekly/20260810_Article_0002.md",
        "20260810_OOS_Bulletin_Weekly/20260810_Article_0003.md",
    ]
...
def test_the_files_sit_in_a_folder_so_two_bundles_unzip_side_by_side(corpus):
    ...
    assert all(n.startswith("20260810_OOS_Bulletin_Weekly/") for n in a)
    assert all(n.startswith("20260810_OOS_Bulletin_Weekly_2/") for n in b)
...
def test_the_reference_in_the_report_opens_the_file_of_the_same_article(corpus):
    ...
    for ref in sorted(set(re.findall(r"`\[(\d{4})\]`", report))):
        name = f"20260810_OOS_Bulletin_Weekly/20260810_Article_{ref}.md"
...
def test_an_article_file_keeps_the_two_classes_of_fact_apart(corpus):
    ...
    body = files["20260810_OOS_Bulletin_Weekly/20260810_Article_0001.md"]
...
def test_an_excerpt_only_bundle_says_so_rather_than_looking_complete(corpus):
    ...
    files = _unzip(out)
    toc = files["20260810_OOS_Bulletin_Weekly/00_Table_of_Contents.md"]
    body = files["20260810_OOS_Bulletin_Weekly/20260810_Article_0001.md"]
...
def test_the_text_budget_degrades_loudly_and_names_the_boundary(corpus):
    ...
    later = files["20260810_OOS_Bulletin_Weekly/20260810_Article_0003.md"]
...
def test_the_contents_page_is_a_table_that_says_where_each_article_is_cited(corpus):
    ...
    assert "`20260810_Article_0002.md`" in toc
...
def test_an_edition_naming_no_articles_produces_an_honest_empty_bundle(corpus):
    ...
    assert list(files) == ["20260810_OOS_Bulletin_Weekly/00_Table_of_Contents.md"]
...
def test_the_report_downloads_under_its_own_name_not_the_records(client):
    ...
    assert 'filename="20260810_OOS_Bulletin_Weekly.md"' in r.headers["content-disposition"]
...
def test_the_annexes_download_as_a_zip_named_after_the_report(client):
    ...
    assert 'filename="20260810_OOS_Bulletin_Weekly_Annexes.zip"' in r.headers[
        "content-disposition"
    ]
...
def test_the_annexes_honour_the_same_selection_as_the_report(client):
    ...
    for ref in refs:
        assert f"20260810_OOS_Bulletin_Weekly/20260810_Article_{ref}.md" in files
