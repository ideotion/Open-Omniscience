...
def list_for_article(article_id: int, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "tags": datestore.for_article(db, article_id),
        "caveat": _CAVEAT,
    }
...
@router.get("/by-date")
def by_date(
    date: str = Query(..., description="ISO date, e.g. 2001-09-11"),
    ...
    items = datestore.articles_for_date(
        db, date_str=date, precision=precision, status=status, limit=limit
    )
