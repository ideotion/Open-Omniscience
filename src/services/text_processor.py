...
class TextProcessor:
    ...
    def tokenize(self, text):
        ...
        words = [
            word for word in words if self.min_word_length <= len(word) <= self.max_word_length
        ]
        return words
    ...
    def process_text(
        self, text, language="en", remove_stopwords=True, include_ngrams=True, ngram_range=(1, 3)
    ):
        if not text:
