...
class KeywordExtractor:
    ...
    def extract_keywords_from_article(
        self, article_text, title="", language="en", title_weight=2.0
    ):
        ...
        filtered_keywords = [
            word
            for word, count in combined_freq.items()
            if count >= self.min_frequency
            and self.min_keyword_length <= len(word) <= self.max_keyword_length
        ]
    ...
    def get_top_keywords(self, text, language="en", top_n=10, include_scores=False):
        ...
        if not include_scores:
            top_keywords = sorted(result["frequencies"].items(), key=lambda x: x[1], reverse=True)[
                :top_n
            ]
            return top_keywords
        else:
    ...
    def extract_key_phrases(
        self, text, language="en", min_phrase_length=2, max_phrase_length=5, top_n=10
    ):
