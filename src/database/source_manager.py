...
class SourceManager:
    ...
    def set_source_rate_limit(self, source_ids: list[int], rate_limit_ms: int) -> int:
        ...
        result = (
            self.session.query(Source)
            .filter(Source.id.in_(source_ids))
            .update({"rate_limit_ms": rate_limit_ms}, synchronize_session=False)
        )
        self.session.commit()
    ...
    def create_tag_based_group(self, name: str, tag_pattern: str, **kwargs) -> SourceGroup:
        ...
        Returns:
            The created SourceGroup object
        """
        group = self.create_group(name=name, is_tag_based=True, tag_pattern=tag_pattern, **kwargs)
    ...
    def enable_groups(self, group_ids: list[int]) -> int:
        ...
        sources = (
            self.session.query(Source)
            .join(source_group_association)
            .filter(source_group_association.c.group_id.in_(group_ids))
            .all()
        )
    ...
    def discover_rss_feeds(self, source_ids: list[int] = None, timeout: int = 10) -> list[dict]:
        ...
        if source_ids:
            ...
            sources = (
                self.session.query(Source)
                .filter(or_(Source.rss_url == None, Source.rss_url == ""))
                .all()
            )
    ...
    def get_source_statistics(self) -> dict[str, Any]:
        ...
        with_rss = (
            self.session.query(Source).filter(Source.rss_url != None, Source.rss_url != "").count()
        )
        ...
        with_country = (
            self.session.query(SourceMetadata).filter(SourceMetadata.country != None).count()
        )
        with_language = (
            self.session.query(SourceMetadata).filter(SourceMetadata.language != None).count()
        )
