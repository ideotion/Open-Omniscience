...
class StatisticalTests:
    ...
    def t_test_independent(
        ...
        pooled_std = np.sqrt(
            ((n1 - 1) * np.var(arr1, ddof=1) + (n2 - 1) * np.var(arr2, ddof=1)) / (n1 + n2 - 2)
        )
    ...
    def t_test_one_sample(
        ...
        effect_size = (
            (np.mean(arr) - popmean) / np.std(arr, ddof=1) if np.std(arr, ddof=1) > 0 else 0
        )
    ...
    def chi_square_goodness_of_fit(
        self, observed: list | np.ndarray, expected: list | np.ndarray | str = "uniform"
    ) -> TestResult:
        """
        ...
        if len(obs) != len(exp):
            raise ValueError(
                f"Observed and expected must have same length. Got {len(obs)} and {len(exp)}"
            )
    ...
    def chi_square_independence(
        self, contingency_table: list | np.ndarray | pd.DataFrame
    ) -> TestResult:
        """
    ...
    def one_way_anova(
        self, *groups: list | np.ndarray | pd.Series, equal_var: bool = True
    ) -> TestResult:
        """
        One-way ANOVA.
    ...
    def two_way_anova(self, data: pd.DataFrame, formula: str) -> dict[str, Any]:
        """
        ...
        if not HAS_STATSMODELS:
            raise ImportError(
                "statsmodels is required for two-way ANOVA. Install with: pip install statsmodels"
            )

        model = ols(formula, data=data).fit()
        anova_table = sm.stats.anova_lm(model, typ=2)

        return {"model": model, "anova_table": anova_table, "summary": model.summary()}

    # ==================== REGRESSION ====================

    def linear_regression(
        self, x: list | np.ndarray | pd.Series, y: list | np.ndarray | pd.Series
    ) -> dict[str, Any]:
        """
    ...
    def multiple_regression(self, data: pd.DataFrame, formula: str) -> dict[str, Any]:
        """
    ...
    def tukey_hsd(
        self, data: list | np.ndarray | pd.Series, groups: list | np.ndarray | pd.Series
    ) -> dict[str, Any]:
        """
        ...
        if not HAS_STATSMODELS:
            raise ImportError(
                "statsmodels is required for Tukey HSD test. Install with: pip install statsmodels"
            )

        result = pairwise_tukeyhsd(endog=data, groups=groups, alpha=self.alpha)
...
def one_way_anova(*groups: list | np.ndarray | pd.Series, equal_var: bool = True) -> TestResult:
    """One-way ANOVA (module-level function)."""
    return _get_default_tests().one_way_anova(*groups, equal_var=equal_var)


def chi_square_independence(contingency_table: list | np.ndarray | pd.DataFrame) -> TestResult:
    """Chi-square test of independence (module-level function)."""
    return _get_default_tests().chi_square_independence(contingency_table)


def pearson_correlation(
    x: list | np.ndarray | pd.Series, y: list | np.ndarray | pd.Series
) -> TestResult:
