...
class ConfidenceInterval:
    ...
    def __repr__(self) -> str:
        return (f"ConfidenceInterval({self.estimate:.4f} [{self.lower:.4f}, {self.upper:.4f}] "
...
class ConfidenceIntervals:
    ...
    def proportion_ci_wilson(
        ...
        z2 = critical ** 2
        numerator = p_hat + z2/(2*trials) - critical * np.sqrt(
            (p_hat*(1-p_hat) + z2/(4*trials))/trials
        )
        denominator = 1 + z2/trials
        
        lower = numerator / denominator
        upper = (p_hat + z2/(2*trials) + critical * np.sqrt(
            (p_hat*(1-p_hat) + z2/(4*trials))/trials
        )) / denominator
    ...
    def diff_proportions_ci(
        ...
        # Standard error
        std_err = np.sqrt(
            p_pooled * (1 - p_pooled) * (1/trials1 + 1/trials2)
        )
    ...
    def summarize_ci(
        ...
        return (f"{ci.estimate:{fmt}} [{ci.lower:{fmt}}, {ci.upper:{fmt}}] "
