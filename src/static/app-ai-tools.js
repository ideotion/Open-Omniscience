...
*/
    ...
    async function _qualScopeCount() {
      ...
      try {
        ...
        const n = r && r.matched;
        // ONE KEYABLE FRAME, not four fragments welded to two numbers. The old form
        // concatenated a lead-in phrase, n, the word for sources, an open parenthesis,
        // the word "of", and total -- an order no locale can change, and whose first and
        // third pieces carried no key at all. A Chromium walk in fr is what showed it,
        // because both i18n ratchets were green over it. `tf()` translates the FRAME and
        // interpolates the DATA afterwards.
        //
        // The old fragments are DESCRIBED above rather than pasted, and that is not
        // squeamishness: --audit-chrome scans raw source, so a quoted literal inside a
        // comment counts as an untranslatable UI string exactly as a live one does. The
        // first draft of this comment pasted the broken form verbatim, which added a
        // phantom entry to the count and sent five translators hunting for a surface
        // that does not exist.
    ...
    async function launchOllama(btn) {
      ...
      try {
        ...
        toast(_failMsg("Ollama: {error}", e), "err");
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = was; }
      }
      loadAiBackendPanel();
    ...
    async function installVllm(btn) {
      const t = (window.OOI18N && OOI18N.t) ? OOI18N.t : ((s) => s);
      if (btn) { btn.disabled = true; btn.textContent = t("Starting the install…"); }
    ...
    async function stopVllm(btn) {
      ...
      try {
        ...
        loadVllmStatusPanel();
        loadLlmHealth();
      } catch (e) { toast(_failMsg("Stop: {error}", e), "err"); }
      finally { if (btn) btn.disabled = false; }
    }
    ...
    async function summarize(id, btn) {
      ...
      } catch (e) { cell.textContent = ""; toast(_failMsg("Summarize: {error}", e), "err"); }
    ...
    async function translateArticle(id, btn) {
      ...
      try {
        ...
        cell.innerHTML = `<span class="muted">[${esc(r.source_language ? ooLangName(r.source_language) : "?")}→${esc(ooLangName(r.target_language))}]</span> `
          + `${esc(r.result)} <span class="muted">— ${esc(r.model)}</span>`
          + _llmMethodNote(r.method)
          + `<div class="hint muted">Generated by a local model — verify against the stored article.</div>`;
      } catch (e) { cell.textContent = ""; toast(_failMsg("Translate: {error}", e), "err"); }
      loadLlmHealth();
    }
    ...
    async function viewChain() {
      ...
      } catch (e) { $("cust-entries").innerHTML = ""; toast(_failMsg("View chain: {error}", e), "err"); }
    }
    ...
    async function verifyChain() {
      ...
      } catch (e) { toast(_failMsg("Verify: {error}", e), "err"); }
    }
    ...
    async function exportCustody() {
      ...
      } catch (e) { toast(_failMsg("Export: {error}", e), "err"); }
    }
    ...
    async function anchorRoot() {
      ...
      } catch (e) { toast(_failMsg("Anchor: {error}", e), "err"); }
    }
