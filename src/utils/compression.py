...
class CompressionStats:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "space_saved_percentage": round(
                ((self.original_size - self.compressed_size) / self.original_size * 100), 2
            ),
        }
...
class Compressor:
    ...
    _ALGORITHMS: dict[CompressionAlgorithm, dict[str, Any]] = {
        ...
        CompressionAlgorithm.LZMA: {
            "compress": lambda data, config: lzma.compress(
                data, format=lzma.FORMAT_XZ, preset=config.level
            ),
    ...
    _DEFAULT_CONFIGS: dict[CompressionAlgorithm, CompressionConfig] = {
        ...
        CompressionAlgorithm.ZSTANDARD: CompressionConfig(
            algorithm=CompressionAlgorithm.ZSTANDARD, level=6
        ),
        CompressionAlgorithm.LZ4: CompressionConfig(algorithm=CompressionAlgorithm.LZ4, level=6),
        CompressionAlgorithm.BLOSC: CompressionConfig(
            algorithm=CompressionAlgorithm.BLOSC, level=6
        ),
        ...
        CompressionAlgorithm.SNAPPY: CompressionConfig(
            algorithm=CompressionAlgorithm.SNAPPY, level=6
        ),
    }
    ...
    def _load_optional_algorithms(self) -> None:
        ...
        try:
            ...
            _note_missing_optional(
                "zstandard", 'pip install ".[compression]"  (or: pip install zstandard)'
            )
        ...
        try:
            ...
            self._ALGORITHMS[CompressionAlgorithm.LZ4] = {
                "compress": lambda data, config: lz4.frame.compress(
                    data, compression_level=config.level
                ),
    ...
    def decompress(
        self, compressed_data: bytes, expected_algorithm: CompressionAlgorithm | None = None
    ) -> bytes:
        """
        Decompress data.
        ...
        if len(compressed_data) < HEADER_SIZE:
            raise CompressionError(
                f"Invalid compressed data: too short ({len(compressed_data)} bytes)"
            )
        ...
        if not self.is_available(algorithm):
            raise CompressionError(
                f"Algorithm {algorithm.value} is not available for decompression"
            )
        ...
        # Create config for this algorithm
        algo_config = CompressionConfig(algorithm=algorithm, level=level)
    ...
    def _create_header(
        self, algorithm: CompressionAlgorithm, level: int, original_size: int, data_hash: bytes
    ) -> bytes:
    ...
    def benchmark(
        self, data: str | bytes, algorithms: list[CompressionAlgorithm] | None = None
    ...
    def get_best_algorithm(
        ...
        if optimize_for == "ratio":
            ...
            best_stat = max(
                stats_list,
                key=lambda x: (
                    x.compression_ratio / (x.compression_time + x.decompression_time + 0.0001)
                ),
            )
...
class DatabaseCompressor:
    ...
    def compress_field(self, field_name: str, value: str | bytes | None) -> bytes | None:
        """
        ...
        if isinstance(value, str):
            algorithm = (
                ...
                if self.compressor.is_available(CompressionAlgorithm.ZSTANDARD)
    ...
    def decompress_field(self, field_name: str, value: bytes | None) -> str | bytes | None:
        """
