...
**Phase 1 (this work):** the i18n engine (`i18n.js`), the locale scaffold for all 12
languages, and complete reference translations for **en/fr/es/de**; the rest are
selectable stubs (English fallback). **Phase 2 (next, needs a browser):** include
`i18n.js` in both interfaces, add the language picker, call `apply()` on render, and
extend `en.json` to the full string catalogue. Honest and incremental.
