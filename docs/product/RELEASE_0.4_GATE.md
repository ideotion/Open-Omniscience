...
| S | Sources: `qualified` ⇒ `enabled`, the hatch retired, the overlay editor, the institutions moves, the Stage B splice, the stratified round-robin | session + operator (the shortlist run) | ruled (Q1101, Q1105–Q1112, Q1114–Q1119, Q1156) · `S04-12` | **OPEN** — S1 + S2 SHIPPED (the flip, the admission audit + undo, the hatch retired; the shipped-verdict editor, the merge action, the labelled headline counts); S3–S4 remain; the embassy platforms (Q1113 ⛔) are PENDING, and nothing here decided them |
