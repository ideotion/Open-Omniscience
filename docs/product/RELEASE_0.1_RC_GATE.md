...
| 100k-article scale proof + consented-archiving design (B3) | ⬜ | SHOULD-proposed | perf-harness profile; measure first. PERF WORKSTREAM (field report 2026-06-18, in flight): insights TTL cache + warm; framing/supergroups bounded; **denormalised `Keyword.mention_count`/`article_count` maintained at index time — slice 1 (columns + incremental maintenance + backfill + self-heal + migration a2b3c4d5e6f7) + slice 2 (top_terms corpus-wide path + _supergroup_totals read the counters, no mention join; trending/map stay windowed/by-country) both SHIPPED** |
