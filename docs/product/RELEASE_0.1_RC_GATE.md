...
> **Reconciled 2026-07-02 (the release execution pass — see `RELEASE_0.1_PLAN.md` for
> the full evidence + arbitration log; maintainer granted full release autonomy the
> same day).** Verified against code/tests at HEAD by a 19-agent sweep + a full local
> py3.13 suite run (2 496 passed · 7 skipped · 0 failed) + hand verification of every
> critical. Rows ADVANCED with evidence: guarded socket factory ✅ (ratchet allowlist =
> reality, 3 deliberate importers); backup-at-scale ✅ (volumes+parity end-to-end,
> 2 GiB single-file CREATE removed, route-composition invariant); unified Import/Export
> ✅; .eml UNIQUE-hash bug ✅ (regression-tested); two-windows consolidation ✅ (openCorpus
> → #an, modal retired); interactive-charts remainder ✅ (commodity enlarge → ooChart);
> Search-tab absorption ✅ (parity honoured); fixity tool ✅; Tor manual chapter +
> transport verdicts ✅; French easter eggs ✅; threat-model statements ✅. NEW BLOCKERS
> found + being fixed on the release branch: wheel/sdist carried ZERO data files
> (release artifacts could not run — packaging fix + fresh-venv boot proof); the mypy
> ratchet was red at HEAD (132>127 — merged-not-green during a CI gap: no completed
> default-branch run since 06-29, all superseded-cancelled; fixed to 127); CHANGES.md
> 0.09 section ~2 weeks stale; custody auto-log UI text claimed a default the code
> didn't set (flipped ON per the Item-N ruling); 4 live-DB self-heal holes for
> 0.0.8→0.1 upgraders; "Back up first" buttons still on the 2 GiB-capped path; the
> 128-defect docs↔app/ethics coherence inventory (both directions). Consciously
> ACCEPTED for 0.1-alpha with reasons recorded in the plan §4: inline-handler
> retirement, a11y batch, i18n chrome tail (~150, all consent surfaces keyed),
> D1/D4 state-into-DB, GUI self-update mechanics (never built), full 463-PR re-audit
> (replaced by the evidence set in plan §3.G), Swagger-CDN exception, win/mac
> (ruling #5). The fork-3 browser debt is being closed HERE by an in-container
> Playwright campaign (first real click-through), not deferred.
...
| The ONE corpora system (6 entries: hand/tag-selection/tag-click/commodity-click/keyword-click/date-keyword-click; keyword windows = same sub-tabs + events sub-tab + TIME-SCOPE control) | ✅ 2026-07-02 (two-windows consolidation verified done; residual: Advanced tag-filter UI + a hand-selection entry, additive conveniences) | RC-BLOCKING (the flagship analysis object) | sub-tab set built out on the `#corpus` modal (Mindmap/Sentiment/Keywords/Sources/Competitive, PRs #214–218) AND the Group F `#analyze` window (invariant #22); TIME-SCOPE shipped as `ooTimeScope` (PRs #197–201). REMAINING: the **two-windows consolidation** (known debt, CLAUDE.md) + the remaining entry points |
| Interactive charts (zoom/pan/X-Y readout/legends; kill the 5-point cap; real curves) | ✅ 2026-07-02 (commodity-card enlarge → ooChart verified) | RC-BLOCKING (live-test complaint) | full series at all scales; **indices board detail rolled onto `ooChart`** (PR #205; invariant #16 asserts `ooChart($("idx-chart-oo")`). **Trends sparkline → click-to-enlarge `ooChart`** via the reusable `chart-enlarge` dialog (Item 1; invariant #21b++). REMAINING: commodity-card enlarge → `ooChart` (the n<10→bar rule, Item Y, SHIPPED app-wide 2026-06-15 in both renderers) |
...
| i18n long tail → ~0 (audit-chrome per tab) + Home-card title translation design | 🔶 ruling #8 (PROCEED) | RC-BLOCKING | keyed strings stay **100% ×12** (`--min 100` green); `--audit-chrome` measures **140 untranslatable chrome strings as of 2026-06-20** (was 431 on 06-17 → burned down through the prior slices + the 2026-06-20 autonomous batch keying 82 more: clean single-text-node labels/paragraphs + 5 cosmetic de-tags). REMAINING tail is mostly inline-`<a>`-linked help paragraphs (need the link-at-end restructure), the few `<strong>`-emphasised privacy warnings (kept emphasised deliberately), and data/examples that stay literal (codes, URLs, proper nouns). Mechanism confirmed: a clean single-text-node string is keyable directly (the walker matches per text node); inline-tagged fragments need de-tagging first. Burn down in native-reviewable slices (AI-drafted non-en, flagged) |
| French easter eggs (transnational, translatable) | ✅ 2026-07-02 | SHOULD | personality.yml |
| Tor transport-awareness in per-host verdicts + "running over Tor" manual chapter (logs pending from maintainer) | ✅ 2026-07-02 | SHOULD | verdicts distinguish robots/Tor-refused/down |
| Global search rework (omnibar absorbs Search tab — only after parity) | ✅ 2026-07-02 (parity honoured before removal) | POST | the Desk lesson: never lose a tool |
...
| Local fixity audit tool (B2 — reliable-memory turned inward) | ✅ 2026-07-02 | SHOULD-proposed | re-hash corpus vs stored hashes, loud report |
| 100k-article scale proof + consented-archiving design (B3) | 🔶 2026-06-19 | SHOULD-proposed | perf-harness profile; measure first. PERF WORKSTREAM (field report 2026-06-18): insights TTL cache + warm; framing/supergroups bounded; **denormalised `Keyword.mention_count`/`article_count` maintained at index time — slice 1 + slice 2 (counters read by top_terms/_supergroup_totals, no mention join) SHIPPED**. **DATA-ARCHITECTURE BUILD 2026-06-19 (AUTONOMOUS_BUILD_BRIEF_DATA_ARCH.md, branch claude/modest-hopper-gisgst, draft PR):** honesty ENVELOPE (`src/analytics/envelope.py`); counter FRESHNESS + bounded background reconcile (`Keyword.last_reconciled_at`, migration b2c3d4e5f6a7; `/top`+`/supergroups` carry an `exact`/`estimated` `counts` envelope); A1 READ-MODEL SEAM (`src/analytics/readmodel.py`, endpoints route through it); DERIVED COLUMNAR engine PR-1 (`src/analytics/columnar.py`, optional `[columnar]` duckdb, encryption-gate + in-memory hard-fallback, never plaintext — persisted-offline blocked on a per-OS httpfs crypto-extension packaging decision); K1/K2 identity (`Article.content_multihash`+`canon_version`, migration d3e4f5a6b7c8); SOURCE-IP capture (`Article.server_ip`/`ip_observed_at`/`server_ip_reason`, migration e4f5a6b7c8d9, clearnet-only/honest-over-Tor) + OFFLINE geolocation (`src/geo/ip_geo.py`, DB-IP CC BY 4.0, `scripts/build_ip_geo.py`) + `GET /api/insights/server-locations` (clustering, no score). **FOLLOW-UP (PR #410, 2026-06-19):** real DB-IP table BUNDLED (CC BY 4.0 mirror, ~4.4 MB, offline lookups proven); ooMap "Server IPs" FRONTEND layer (browser-unverified); columnar read-model BUILDER + byte-identical projection + cold-fallback; persisted background maintenance + `GET /api/diagnostics/columnar` observability. **PR-3 (2026-06-19):** the heavy-aggregation perf landed via the Slice-2 COUNTERS, not DuckDB — `associations` (the 76 s endpoint) de-N+1'd (batch-load co-keywords + n_b from `article_count` corpus-wide / one grouped query windowed; byte-identical, `test_associations_perf.py`); `layered_graph` keyword-level inherits it; `framing` already bounded. REMAINING: the per-OS httpfs crypto-extension packaging decision (enables persisted-offline encryption + the optional columnar acceleration of the inherent co_rows GROUP BY); the 100k empirical run |
