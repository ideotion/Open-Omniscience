...
| RM-08 | packaging | Offline wheelhouse installer + `.deb` / reproducible build for air-gapped & Qubes; **incl. the "offline LLM kit"** (checksummed release artifact: Ollama binary + a small model, USB-transferable to ~/.ollama/models) for operators who can never use clearnet | Air-gapped classrooms and at-risk installs (5) | — | M | Strengthens local-first; reproducible = auditable |
...
> the live-test hardening batches); the "deeper sense-making" horizon shipped as
> **`0.1.0`, the first alpha** (default branch renamed `0.09 → 0.1` at the flip).
