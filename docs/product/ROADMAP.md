...
| RM-20 | capability | **Investigation recipes** — turn the ten space-time scenarios into Home-screen cards: a producer per scenario + an optional `recipe` (`{tab, params}` deep-link) on `Card` that renders an "Open this investigation" button pre-filling the relevant tab. Active recipes are a Settings choice (see `USE_CASES.md` §"From scenarios to Home-screen cards") | One-click investigations on the home screen; the app does the legwork, the user keeps the judgement (all personas) | — | M | Reuses the existing producer/registry/`Card` engine incl. the `CardSchemaError` no-composite-score guard; a recipe only pre-fills a query the user could type — automates gathering, never the verdict; cards dismissible & user-owned |
