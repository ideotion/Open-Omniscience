...
**Remaining (lower priority — tracked for follow-up):**
- P2: ~40 `source_management`/`keyword_analysis` endpoints use `SourceManager()`/
  `get_session()` instead of `Depends(get_db)` (works in prod — same engine — but not
  request-scoped/test-overridable).
- P2: per-router `Limiter()` instances aren't attached to the app, so router-level
  `@limiter.limit` decorators are unenforced (low impact on a loopback app).
- P2: `verification` reads the whole upload before the size check; `monitoring` checks
  sources sequentially with no cap; `utils/cache.py` `@cache` raises on unhashable args;
  `url_utils` drops multi-valued query params; `duckduckgo._is_xml_content` regex;
  `merkle_tree` lacks leaf/internal domain separation; keyword handlers are `async def`
  doing blocking CPU work.
- P2 (latent, not on the live path): `utils/compression.py` single-byte algorithm-id
  collision (`Article.content` is plain Text, so not currently exercised).
- P3: assorted (stale `.env.production.example`, `package/`, `monitoring/grafana`,
  several `docs/*_GUIDE.md` still referencing removed features).
