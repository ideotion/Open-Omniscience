...
| SRC-11 | 54 duplicate domains → 227 of 3,429 `configs/sources.yml` entries unreachable by the create-only seeder | GUARD BUILT 2026-09-07; the RECOVERY is RULING-GATED (new: B11). The "data fix" direction was wrong — measured, 108 of the 227 are in a DIFFERENT language than the surviving sibling (30 BBC language services, DW Arabic/Deutsch/Español/Brasil), so deleting them deletes the multilingual breadth; recovery needs a source-identity ruling (domain vs feed) | `catalog_domain_collisions`; `tests/test_catalog_domain_collisions.py` | CLAUDE.md 2026-09-05 lesson | P04 |
