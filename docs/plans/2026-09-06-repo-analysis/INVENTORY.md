...
| UI-03 | Inline `on*=` handlers, re-measured 2026-09-08: 332 in `index.html` (247 `onclick`, 53 `onchange`, 17 `onkeydown`, 14 `oninput`, 1 `ontoggle`) + 260 across the 18 `app-*.js` modules (16 of the 18 carry at least one) = 592; CSP `'unsafe-inline'` | UNBUILT (H4) | `grep -o " on[a-z]*=\""` counts; `src/api/main.py:555` | ROADMAP/PARKED | P15 |
...
| NET-04 | Nonce-based CSP (blocked on inline-handler retirement — the blocker is 592 handlers as of 2026-09-08, 332 in `index.html` + 260 across the 18 `app-*.js` modules; landing the nonce first breaks the app) | UNBUILT (sequenced behind P15 S2) | `src/api/main.py::_CSP` `script-src 'self' 'unsafe-inline'` | audit S-006 residual | P21 |
...
button) · REL-04 (the "100 GB" strings) ·
PRH-18 (install docs) · the four "PENDING execution" brief banners whose briefs were executed within 48 hours.
(UI-03's own count was re-measured and corrected 2026-09-08 — the row now reads 592, matching the tree; no
discrepancy remains.)
