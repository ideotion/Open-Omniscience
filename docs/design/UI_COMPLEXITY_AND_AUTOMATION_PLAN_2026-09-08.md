...
## 11. The replacement task-first proposal (commissioned separately)

> The third proposal angle in the panel above returned a placeholder and scored 2–3/100. That angle —
> **task-first / verb-first** — was re-commissioned as a standalone agent after the panel had closed, so
> it carries **no judge scores** and was **not** part of the adjudication in §6. It is reproduced here in
> full because the angle was a designed part of the comparison and dropping it silently would leave the
> record thinner than it should be. Read it as a third option on the table, not as a scored contender.
>
> Its own strongest claim is worth flagging: it argues that all three named feature gaps from the
> discoverability test (compare two commodities, find under-sourced topics, verify custody) are a
> noun/verb mismatch rather than a depth or automation problem — and that `docs/DESIGN.md`'s own 2026-06
> Desk critique already argued this, with the maintainer's verdict saying it should eventually be
> "folded into the Console". If that reading of the existing design record is right, it is a
> consideration the other two proposals do not engage with at all.

# Verbs, not nouns — a task-first information architecture for Open Omniscience

## Thesis

Open Omniscience's sidebar answers the question "which module has this?" when the question a working journalist actually holds is "what am I trying to do right now?" — and the discoverability audit shows the gap between those two questions is exactly where the app fails: the one custody-verification failure it found is not that custody is hidden deep (rings-not-gates' diagnosis) or that it requires a manual click the app could take for you (works-while-you-sleep's diagnosis) — it is that a user standing in the *reader*, wanting to *check a claim*, has no reason to guess that the answer lives under a noun called "Advanced → Safety" four clicks away, because "check a claim" and "Advanced → Safety" are not the same sentence in any user's head. The same mismatch produces the other two named gaps: "compare two commodities" fails because Commodities and Indices are different nouns even though *comparing two series* is one verb the app already built once (Indices' own compare bar); "find under-sourced topics" fails because the one measure that answers it lives inside a noun called "Stats" as a JSON download instead of inside whatever verb a journalist would call that question. This proposal reorganizes the primary navigation around eight investigative verbs — Read, Watch, Compare, Corroborate, Trace an origin, Check a claim, Follow a story, Publish — and turns the current 16 noun-surfaces into lenses, tune-drawers, and shared components mounted *inside* those verbs, so that the sidebar and the sentence in the user's head are finally the same sentence. **Task-first beats a disclosure ladder and an automation layer on the maintainer's actual, measured problem because both of those cuts leave the noun-to-verb translation in place — Rings changes how much of the noun list is pinned, and Sleep changes how many clicks a noun-shaped action takes, but a fully-automated, perfectly-graded list of 16 nouns is still a list of 16 nouns a user must map their question onto; only reorganizing by verb removes the mapping step itself**, which is also, concretely, the only one of the three cuts that closes the audit's three named feature gaps as a direct structural consequence rather than as added scope. It is also not a new idea for this codebase: `docs/DESIGN.md`'s own 2026-06 "Console vs Desk" argument made exactly this case ("Amara doesn't think in modules; she thinks in a case") and the maintainer's final verdict, while retiring Desk's *chrome*, explicitly instructed that "Desk's best ideas — the task-framed home... — should be folded into the Console over time." This proposal is that fold, carried all the way through, not stopped at a mood.

## The proposal

### 0. What doesn't change

Console's chrome verdict (2026-06-10) is left untouched: same collapsible sidebar mechanic, same top bar, same 17-theme catalog, same density/font controls, same command palette, same brand mark. This is not Desk reborn — nothing here argues for less chrome, fewer themes, or a calmer aesthetic. The only thing that moves is **what the sidebar's list is a list *of*** — verbs instead of nouns — and where each existing panel is mounted underneath that list.

### 1. The task set

Eight destinations replace the current ten (Home…Library) in the sidebar, in this order, each opened by `showTab()` exactly as today:

| # | Task | The question it answers |
|---|---|---|
| — | **Start here** | "What happened, and what needs me?" (the front door; not itself an investigative verb) |
| 1 | **Read** | "What's new, and what does it mean?" |
| 2 | **Watch** | "What am I keeping an eye on, and how well?" |
| 3 | **Compare** | "How does A stack up against B?" |
| 4 | **Corroborate** | "Is this actually confirmed, or does it just look that way?" |
| 5 | **Trace an origin** | "Where did this actually come from?" |
| 6 | **Check a claim** | "Can I prove this hasn't been altered?" |
| 7 | **Follow a story** | "How has this developed over time?" |
| 8 | **Publish** | "What do I hand to an editor?" |

Settings stays pinned at the sidebar footer exactly where it is today; Help stays a top-bar `?` icon exactly where it is today. Search stops being a destination and becomes the omnibox's full-time job (§4). Total pinned destinations: 9 (was 10) — this proposal does not claim to shrink the sidebar; it claims to relabel it correctly, and says so plainly in "What it costs."

### 2. What each task surface contains

**Start here.** Not a dashboard of everything — a one-line teaser per task, each teaser pulled live from that task's own ambient status (§5) and linking straight into it: *"Corroborate: 1 new Overtold cluster, 3 Undertold · Follow a story: 1 item resurfaced after 319 days · Watch: 7 candidate sources awaiting review, 3,598 pending."* This is the *only* place all eight tasks are summarized together, and it is intentionally thin — full Lead cards (with their flip-to-caveat mechanic, unchanged) render inside the task they actually belong to, not here. The persistent corpus-stat band (`453 ARTICLES · 3,618 SOURCES · …`), currently rendered only on Home, is promoted to a slim, collapsible strip under the top bar on **every** task page — this is Library's orientation role generalized app-wide rather than deleted (§3, Library).

**Read.** Two lenses via `ooSubtabs`: **Notable** (the former Home content that is genuinely about "what's worth reading right now" — Context-family cards, Trending-now, By-channel) and **Everything** (today's Feed verbatim — shuffled/newest-first/reshuffle, the "83 held back" caveat unchanged). Opening any article still opens the existing reader tab bar (Read · Summary · Translation · Keywords · Mindmap · Sentiment · Related · Source · Links), which gains one new tab, **Verify** (§3, Custody), directly answering discoverability failure #17.

**Watch.** The app's radar. Default view: what's being watched and how completely — source list + admission status (qualified/pending/candidate), per-tag coverage (the reach/fresh/never-reached/backed-off breakdown that today lives only in the orphaned task-manager dialog), tracked Wikipedia pages, tracked indices/commodities, tracked laws, world coverage. A "Manage in bulk →" panel surfaces the entire zero-UI Source Groups + batch enable/disable/priority/rate-limit + RSS/topic-discovery feature (24 backend routes, confirmed unwired) as Watch's power-user layer — this is a bulk *editor* for a human's own judgment, never an autonomous promoter (source admission still never auto-enables, per `discover.py`'s own boundary). "Add a source," "Watch a Wikipedia page," and the calendar-feed directory move here from Settings.

**Compare.** One canonical multi-select-and-overlay component, generalized from Indices' already-built `_idxCompare`/"Compare (n)" bar so the same control drives commodity-vs-commodity, commodity-vs-coverage (today's single-series Analyze → Price subtab, extended), country-vs-country (Governments' existing "Compare" subtab, wired in verbatim), and keyword-trend-vs-trend (today's Insights → Trends, which stops being Explore's confusing same-named sibling and becomes what its name always promised). This single fix closes discoverability gap #16 (two commodities vs. coverage) directly. An optional "Test this" action on any overlay runs the already-built, already-tested statistics engine (t-test/ANOVA/Mann-Whitney/Pearson/Spearman/CI) and renders statistic + p-value + dof + effect size + n — never a score, exactly as `src/analysis/` already computes it; this closes the "100% unwired statistics engine" finding by giving it the one home where "is this difference real" is literally the question being asked.

**Corroborate.** Default view: the Overtold/Undertold/lonely-signal/source-laundering Lead families (moved here from Home, unchanged internally — same card schema, same flip-to-caveat, same "Ranked by a disclosed order" line), plus Insights → Sources' citation graph ("who cites whom," most-cited external URLs/domains), plus Integrity's coordination-scan. A new lens, **Under-covered**, renders the existing FDR-corrected "bury" signal (today a raw JSON download in Settings → Advanced → Stats) as a sortable table — topic, distinct-source count, significance — closing discoverability gap #18 directly, with zero new backend logic. The three currently-endpoint-less Conjunction-Lens functions that exist only as code (`per_article_intensity`, `conditional_trend`, `vocabulary_contrast`) get real endpoints here, since "is this term over- or under-represented relative to its baseline" is precisely a corroboration question.

**Trace an origin.** The DESIGN.md "content-analysis strategy" vision, wired to what's actually built: Insights → Links' `top-cited`/`articles-by-link` endpoints presented as a citation graph you can pivot from any article, plus (once built — flagged as future work below, not claimed as shipped) the P3 lineage chain (primary document → first report → echoes) the design doc already specifies but the P1 evidence base confirms is UI-less today. World map mounts here as a shared lens ("everything that originated near this place"), not as a separate destination.

**Check a claim.** Custody and Integrity, promoted from *retired-from-sidebar, palette-only* (confirmed by the capability audit: five surfaces including Custody and Integrity have zero sidebar presence today) to a **permanent pinned destination** — a direct reversal of their current demotion, and the fix for the audit's #1-ranked discoverability failure. Default view: paste-or-arrive-from-reader Item ID, View chain / Verify / Export bundle, Anchor root — unchanged controls, better address. The reader's new **Verify** tab (§2, Read) pre-fills the Item ID from the open article and shows a passive result — "✓ Matches what was logged at ingest" / "Not yet checked this session" — never a fabricated "tampered" claim, matching the no-fabricated-security rule exactly.

**Follow a story.** Temporal tracking: Agenda's deduced + imported events (unchanged internally, invariant #13b's pipeline untouched), the Keep-watching/Resurfaced/Recycled-claim Lead family (moved here from Home — this is precisely a "how has this developed" question, not a corroboration question), and Observatory mounted as an optional visual lens ("see your corpus as a night sky, and where it moved since last time") — this *literalizes* Observatory's own invariant #31 language ("a lens, never a second source of truth") rather than contradicting it. Governments' tracked-law change history lands here too, once the dead "official ↗" links (finding LAW-1) are fixed as a prerequisite — a real, fixable bug this task inherits rather than papers over.

**Publish.** The terminal task. The already-present but Home-buried "🛒 Draft (0) →" mechanism becomes this task's primary object: add evidence to a draft from any other task's cards, then Export signed evidence / Methods appendix / Export CSV·JSON / Synthesize·Summarize·Translate-all — all controls that exist today, scattered across Search, Custody, and Insights toolbars, consolidated into the one task whose entire job is producing something to hand to Elliot-the-editor. **This task can never run itself** — see "Why it survives," §2.

### 3. What happens to each of the 16 current surfaces

| Surface | Disposition |
|---|---|
| **Home** | Dissolves as a destination. Lead families redistribute (Overtold/Undertold → Corroborate; Keep-watching/Context/Trending → Follow a story / Read); the corpus-stat band generalizes to a persistent strip on every task; the Draft mechanism seeds Publish; zero-click orientation is preserved via Start here's teaser line, not lost. |
| **Feed** | Becomes Read's "Everything" lens, verbatim. |
| **Insights** | Dissolves as a destination. Explore/Trends → Compare; Sources (citation graph) → Corroborate + Trace an origin; Families/Super-groups → Watch's tune drawer + Trace an origin; Map → the shared World map lens; Convergence → Corroborate; Watches → Watch. |
| **Observatory** | Becomes a shared visual lens opened from Follow a story and Start here — no longer a pinned destination (a literal reading of its own "lens, never a second source of truth" invariant). |
| **World map** | Becomes a shared map lens opened from Follow a story, Trace an origin, and Search results — no longer a pinned destination. |
| **Governments (law)** | Countries/Statistics/Groups → Watch (what per-country data is tracked); Compare subtab → wired verbatim into the Compare task; Law (tracked laws) → Watch + Follow a story + Trace an origin, contingent on fixing LAW-1's dead links. |
| **Agenda** | Becomes Follow a story's primary lens, internals unchanged. |
| **Indices** | Watch (what's tracked) + Compare (its existing multi-series overlay becomes the template Compare reuses everywhere). |
| **Commodities (markets)** | Watch (feed/source config) + Compare (price-vs-price and price-vs-coverage, closing gap #16). |
| **Library** | Dissolves. Corpus-stat band promotes to a global persistent strip; world-coverage view → Watch; backup/export/storage → Settings → Data & Publish's export step. |
| **Settings** | Shrinks to Appearance · General (with an always-expanded Safety area, never folded) · AI & models · Data & backup · Maintainer/diagnostics. Cards, Wikipedia, OfflineMap, the Agenda calendar directory, and all 11 Advanced folds redistribute into each task's own "Tune this task" drawer (§5). |
| **Search** | Stops being a destination; becomes the always-available top-bar omnibox plus a full results workspace whose rows gain "Send to: Compare / Corroborate / Check a claim / Publish" actions — the connective tissue between tasks, not a task itself. |
| **Analyze** | Unchanged in spirit — invariant #22 already says "never a sidebar entry." Extended (a small, explicit amendment, §"What it costs") to be openable from any task's own "Analyze this set →" action, not only Search's. |
| **Custody** | Promoted from palette-only retirement into Check a claim's core content; gains the reader-side Verify entry point. |
| **Integrity** | Promoted alongside Custody into Check a claim's second lens (coordination-scan). |
| **Help** | Mechanically unchanged (top-bar `?`, in-app manual reader); gains task-scoped deep links so opening Help from inside a task jumps to that task's manual section instead of the flat table of contents. |

The pop-up **task manager** (not one of the 16 documented surfaces, but the single most evidence-heavy finding in the capability audit) is reconciled onto its one actually-reachable implementation and **renamed "Engine room."** This is not cosmetic: once "task" is this proposal's own primary IA word, a second, differently-tabbed surface also called "the task manager" is a naming collision this design cannot ship alongside. Engine room absorbs the restored Coverage tab (ported from the orphaned dialog's already-correct rendering logic) and is linked from Watch (since per-tag reach is exactly Watch's business) and from the top bar's existing icon, unchanged in position.

### 4. Search's new role

Search is deliberately *not* task 9. It is the raw-material assembler every task draws from: type into the omnibox from anywhere, get a results workspace (query grammar, filters, export — all unchanged), and from there choose which task to pursue the result set with. This preserves Search's existing "Analyze →" pattern and generalizes it, rather than inventing a new mechanism.

### 5. The automation set this design assumes

Task-first does not introduce a global Activity Ledger (that is the other proposal's mechanism) or a graded pinning dial (that is the third). Its disclosure principle is **task-scoped ambient status**: every task carries its own always-visible "since you last opened this" line, built from work the app already does automatically (the seven scheduler ride-alongs, auto-indexing, auto-clustering) — reported *only* inside the task that would act on it, never in a separate global feed:

- Watch's status line: sources judged since last visit, admitted/rejected counts, discovery candidates staged, coverage deltas.
- Corroborate's status line: new Overtold/Undertold clusters, new near-duplicate groups.
- Follow a story's status line: resurfaced/recycled items, new deduced events.
- Check a claim: nothing ambient (verification is always a deliberate act, never a background one).

This inherits the ethical-screen's own 14-GREEN / 6-AMBER / 7-RED rulings without re-litigating them — GREEN items (qualification, discovery-staging, indexing, near-dup clustering, baseline tagging, housekeeping, coverage-reordering, custody auto-verify-badge, background AI enrichment, bury-signal computation) populate task defaults silently; AMBER items (reindex-backlog auto-resume, background-AI default-on, automatic-backend pre-selection) keep their one-time named disclosure, now shown inside whichever task they affect rather than in a general settings toast; RED items (auto-dismiss a Lead, auto-promote a source, auto-publish a bulletin) stay permanently manual — and the existence of a dedicated **Publish** task is itself the argument for why the last of those three can never be automated: publishing is the one verb this entire IA exists to keep as a deliberate, named human act.

### 6. The disclosure model

Not a global Ring dial, and not "most controls vanish because automation replaced them." Each task gets exactly one **"Tune this task"** drawer — always in the same place, always one click, never nested past that one click — holding only the settings that task actually uses:

- Watch's drawer: per-source rate limits, crawl depth/pages, discovery budget, quality-gate thresholds, the calendar-feed directory (moved here rather than to a generic "Collect & sources" bucket, because Watch — not a Settings label — is the task the directory actually feeds).
- Corroborate's drawer: the Overtold/Undertold/lonely-signal numeric thresholds (z-scores, cohort floors) that today sit inside the 55-control Cards subtab; the family on/off toggle itself stays a first-class, always-visible control on Corroborate's own page, never drawer-gated (per the ethical screen's own R6 ruling that a content-inclusion category must never be collapsed away).
- Compare's drawer: default comparison windows, which series types are eligible.
- Follow a story's drawer: the Keep-watching/resurfaced thresholds, the calendar directory's display options.
- Check a claim's drawer: PQC/OpenTimestamps checkboxes, default actor.

Total control count does not shrink under this proposal — it relocates. This is a deliberate, stated limit, not an oversight (see "What it costs").

### Ordered migration path (each phase independently shippable and coherent on its own)

**Phase 1 — Front door only, fully additive.** Ship "Start here" as a *new* sidebar entry above the existing, untouched ten. It aggregates one-line teasers pointing at today's existing surfaces (no restructuring yet). Ship the reader's Verify tab (fixes gap #17 standalone). Reconcile the two task-manager implementations and rename to "Engine room." Nothing is removed; nothing existing changes shape.

**Phase 2 — Build the eight task pages as thin routers over existing, working panels**, reusing the exact engineering stance `docs/DESIGN.md` already used successfully for the 0.05 redesign ("every original element ID preserved so nothing regresses" — build the shell on top of the proven engine, don't rewrite it). Ship behind an opt-in, reversible "Try task view" toggle in Settings → General, alongside the unchanged noun sidebar. Sequence the three highest-value pages first (see "First three changes").

**Phase 3 — Cut over.** Task view becomes the default for *new installs only* — an existing install is never retroactively switched, mirroring the same humility the rings-not-gates proposal applies to its own Ring default. Settings' Advanced fold is dismantled into per-task tune drawers. The noun sidebar is kept for one further release as "Classic view" (the same gentle-deprecation pattern this project already used for Desk → Console) before removal. Before Phase 3 ships, **re-run the 24-goal discoverability test against the task-view build** — this is not optional, given what is being risked (see "What it costs").

## What it costs

**It does not reduce raw control count, and a user who wants fewer things on screen — not better-organized things — gets less from this proposal than from either competitor.** Rings-not-gates deletes nothing from view by default and lets a user shrink the *pinned* set to almost nothing; works-while-you-sleep deletes buttons outright by making the action unnecessary. This proposal relocates the same ~1,684 Advanced-fold controls into eight smaller, better-named drawers — a real improvement for *finding* the right control, no improvement at all for someone who simply wants fewer controls to exist. Say so plainly: this is a relevance fix, not a complexity-reduction fix.

**It puts the single highest-measured strength in the whole audit at real risk.** Home's front-loaded briefing answers 4 of 24 realistic goals in zero clicks — the strongest engineered defense against cognitive load the audit found anywhere. Splitting that content across seven destinations, with only a one-line teaser at the front door, could regress that strength if the teaser is worse than the full card it replaces. The mitigation is stated as a hard gate, not a hope: Phase 3 does not ship until the 24-goal test is re-run against the task-view build and the median action count and zero-click answer count are shown not to have regressed.

**A power user who has already memorized "16 nouns = 16 places" pays a one-time relearning tax**, and — more seriously — loses the "one screen with everything, comprehensively, for an open-ended audit of my corpus" mode that a flat noun list happens to support well: Daniel wants to browse *everything Insights knows*, not decide which of eight verbs his open-ended curiosity belongs to. "Classic view" (kept for one release per the migration path) is the honest concession to this, not a full answer to it — a truly comprehensive "everything" view may need to survive past the deprecation window as a permanent alternate, and this proposal does not resolve that tension, only defers it.

**Translation load, stated exactly as asked:** roughly 70 new or materially changed user-facing strings — 8 task names, 8 one-line task descriptions, ~8 Start-here teaser templates, 8 "Tune this task" drawer labels, the Engine-room rename, and ~30 relabeled section headings where a noun heading becomes a task-scoped one. At ×12 locales that is **~840 new translation strings**. This is a one-time chrome-label cost, not a per-content cost: every Lead card's own caveat text is unchanged, unchanged in count, and unchanged in its ×12 coverage — only the wrapper around where it's mounted is new.

**The 8-skin GUI gallery (invariant #30) needs real, non-trivial follow-up work.** Each alternate skin (Aurora, Atlas, Command, Canvas, Field, Focus, Terminal, Editorial) currently reuses the one shared `id`-targeted render logic against the noun-shaped `NAV`. A task-shaped rewrite of that navigation either needs each skin re-authored against the new structure (real work, not flagged as done here) or the gallery stays pinned to "Classic view" only for the length of the transition — a real, stated degradation for a feature the ledger has been building toward promoting past "experimental."

**Invariants that need amending, and the exact proposed wording:**

- **Invariant #2** (sidebar lists all tabs). No contradiction in letter — a verb is still a tab — but the word "tabs" currently means *surfaces*, and this proposal makes it mean *tasks that mount several former surfaces each*. Amend for clarity: *"The left sidebar lists every primary destination — task-based as of the 0.4 cycle, formerly surface-based — and stays visible; it may collapse to an icon rail... but must never disappear off-canvas above 600px width."* Argued because leaving the word "tabs" ambiguous between two IA generations is itself a future ledger-repeat risk the maintainer's own protocol (rule 3) warns against.
- **Invariant #13** (calendar-feed directory location, "moved... into Advanced"). Since "Advanced" as a destination dissolves under this proposal, amend: *"...the directory lives in Follow a story's own 'Tune this task' drawer — the task whose events it actually feeds — never in the Agenda lens itself and never in a generic Settings fold."* Argued because this is the *same* reasoning the 2026-07-31 amendment already used (it's the catalogue that feeds the thing, not the thing), pointed at a more specific, more literal destination than "Advanced" ever was.
- **Invariant #19** (Home is content-first, card families as vertical subtabs). Home no longer exists as a destination. Amend: *"The former Home content is preserved as each Lead family's default view inside the task it actually answers (Corroborate for Overtold/Undertold, Follow a story for Keep-watching/Context); zero-click orientation is preserved via a one-line-per-task teaser on the 'Start here' front door, which must never render blank — the fail-safe producer-registration and explanatory-empty-state rule applies per task, not once globally."* Argued because this *strengthens* the original non-negotiable (eight blank-state checks instead of one) rather than weakening it, and because the alternative — leaving #19 describing a page that no longer exists — is exactly the silent-drift failure mode CLAUDE.md's own protocol rule 4 exists to catch.
- **Invariant #20/#20b–e** (task manager). Already measurably broken today (the enforced test watches dead markup; the reachable page has a different tab set). Amend to name the reconciled, renamed surface: *"The system-activity window is served at `/tasks` and opened by the top-bar icon; it is named 'Engine room' — never 'task manager' — to avoid colliding with the task-based primary navigation introduced in the 0.4 cycle. Its tabs are Active · Queue · Schedule · Coverage · System. `test_ui_invariants` must assert against the actually-served page, not any orphaned in-page markup."*
- **Invariant #21** (Insights auto-indexes). "Insights" no longer names a destination. Amend: *"Indexing is a property of the corpus, not of any single surface — it runs automatically regardless of which task is open, and any task reading the index (Compare, Corroborate, Trace an origin, Read) shows the same live 'N to index' count."*
- **Invariant #22** (analysis window "opened from the Search tab's Analyze button"). Minor, low-risk extension: *"...opened from the Search tab's Analyze button, or from any task's own 'Analyze this set →' action — always the same window and endpoint, never a sidebar entry."* No other part of #22 changes.
- **Invariant #31** (Observatory is a lens). No amendment needed — this proposal is the first design to take that invariant completely literally, which is itself evidence the wording was already right.

## Why it survives the constitution

The strongest objection is not "this is a big rewrite" — it is: **redistributing Home's Lead cards across seven destinations multiplies the number of places a caveat could quietly go missing, which is exactly the failure this project's ledger says already happened once** (the Wikipedia dropdown regression that invariant #1 exists to prevent), and does so for a navigational preference, not for honesty.

That objection mistakes *where a component is mounted* for *what enforces its behavior*. The caveat-visibility guarantee — `CardSchemaError`, the flip-card mechanic, the repo-invariant test that checks `.card-caveat` renders outside the hidden `.mc` block — is attached to the **Lead-card component and its own test**, not to the page that happens to import it. Moving a card's parent list from `renderHome()` to `renderCorroborate()` does not touch the function that decides whether its caveat is visible; it is the same component, the same schema, the same test, called from a different caller. Distributing mount points multiplies the *number of pages that display a Lead card*, not the *number of places the caveat-visibility rule must be independently re-implemented and could be independently forgotten* — that number stays exactly one, because it always lived in the component, not the page. This is precisely why `test_ui_invariants` can be extended for #19 in one place (assert the fail-safe applies to Start-here's teaser generation and to each task's own empty state) rather than eight independent, driftable places.

The second half of the objection — "for a navigational preference, not for honesty" — is answered by the three named feature gaps this design closes as a structural side effect rather than as scope creep: a user who could not find custody verification, could not compare two commodities, and could not find under-sourced topics is a user who, in the current architecture, cannot exercise the informed judgment the constitution says is theirs to make ("the app gathers and measures; you judge") — because the measurement existed and the judgment surface did not. Making those three measurements reachable is not a navigational nicety; it is closing a gap between "gathers and measures" and "you judge" that today stops short for exactly the three things the app's own audit went looking for.

## First three changes

1. **Ship "Start here" additively, with the reader's Verify tab and the Engine-room task-manager consolidation riding along.** Zero deletions, zero restructuring of any existing surface, fixes discoverability gap #17 outright, and resolves the "task" naming collision before any other task page exists to collide with it.
2. **Ship Compare as a real page**, generalizing Indices' existing multi-series overlay to Commodities and to Governments' existing country-Compare subtab. This closes discoverability gap #16 by wiring three already-built pieces together behind one new page — no new chart logic, no new backend.
3. **Ship Corroborate as a real page**, moving the Overtold/Undertold Lead families from Home unchanged, adding the citation-graph view from Insights → Sources, and rendering the existing bury-signal endpoint as a sortable table for the first time. This closes discoverability gap #18 with zero new backend logic and gives the second-most sidebar-demoted capability (Integrity's neighbor, Custody, follows in the very next slice) its first permanent, pinned home.
