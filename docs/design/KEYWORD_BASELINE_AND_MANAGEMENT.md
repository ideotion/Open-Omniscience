> **Status update (2026-09-07, docs-hygiene + reality-check pass) — one of the two blockers below is
> now CLOSED, and this doc still stays out of the archive on the strength of the other.** Q3/S1b
> (migrating `_EXTRA_STOPWORD_TEXT` out of a Python string blob into per-language data files) is
> **DONE**: `configs/stopwords_extra/` holds 35 `<lang>.yml` files with a `PROVENANCE.md` evidence
> trail, unioned by `src/analytics/extract.py`'s `_STOPWORDS_EXTRA_DIR` loader. **S4 — an in-app
> one-click apply of the offline analyzer's proposed stopword / ring / mistag candidates — remains
> genuinely unbuilt**, and it is the reason this file is still the design of record rather than an
> archive entry. Read the S4 section for the standing propose-never-auto-apply constraint that
> governs whoever builds it.
