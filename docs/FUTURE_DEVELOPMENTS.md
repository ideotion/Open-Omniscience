...
>
> **AMENDED 2026-09-07 (PROMPT 23 rulings V1-2 / V1-3 / V1-8):** still designed-only for rosters and polls, but the calendar half now has code — see `src/civic/`. **V1-8: the sourced worldwide CALENDAR at the ruled coverage floor is the 1.0 bar; candidate rosters and the poll Tier-2 method audit are POST-1.0.** **V1-3 (exclude restrictive licences entirely) removes IFES ElectionGuide**, which the design named as the freshness half of the layered calendar — it is non-commercial and non-redistributable. The layering is therefore TWO sources, not three: the Wikidata CC0 snapshot + the hand-curated entries + the parallel networked session's sourced recurrence-rule research. **Snap elections are the stated, known gap in the 1.0 calendar** (ElectionGuide was the source that covered them), and the `projected` tier's "check the official source" caveat is what carries it honestly. **V1-2 (no user-supplied API keys) removes OpenFEC**, so US campaign finance is out of scope.
...
>
> **CORRECTED 2026-09-07 (PROMPT 23 staleness guard, tree-anchored `main` @ 965e3e54) — the banner directly above was WRONG about two of its three "genuinely open" items, and is kept rather than rewritten because a status line that was wrong is itself the finding. It is 8 of 9, not 6.**
> **The BURY half of #4 SHIPS:** `buried_topic` (`src/briefing/producers.py`), `find_buried_topics` + `BURY_CAVEAT` (`src/analytics/concentration.py`), registered in `_PRODUCERS`, catalogued in `src/briefing/catalog.py`, exploration surface in `src/api/signals.py` — landed in **PR #568** (merge `1287630f`), i.e. it had been in the tree for over two months when the banner above was written. The "needs a real external trigger" blocker was resolved by a substitution this section did not anticipate: **the rest of the corpus IS the trigger** — a two-proportion z-test of the source's topic share against the rest-of-corpus share, BH-FDR corrected across the (source, topic) family, with specialization (a different beat, region or language) stated as the innocent explanation. Amend §4's "alongside a real external trigger" accordingly.
> **Outrage-intensity (#8) SHIPS too:** `src/analytics/outrage.py`, wired as an annotation inside `src/analytics/headline_body.py` and never a standalone Lead — which IS #8's own spec above ("SECONDARY — annotates, never fires alone"). Listing it as open confused *secondary by design* with *not built*.
> **Genuinely open: #9 alone** (event-timed operation), and per the 2026-09-07 V1-8 ruling — elections ships its CALENDAR at 1.0, rosters post-1.0 — it is now blocked past 1.0 by ruling rather than by neglect, since #9 composes #3 + #6 + the agenda candidate roster.
