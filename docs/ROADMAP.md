...
- **The card section's period** 🚧 (2026-09-07) — `run_all_bounded` takes an `as_of` and hands it to the producers that declare one; five do, and every card states which window its figures came from. Converting the rest is ordinary work behind an existing seam.
- **REMAINING** — a maintainer click-through of the Settings section and review screen 🛠 (every frontend slice shipped browser-unverified per fork-3) · the §6.3 time budget still rests on a guess until `/llm-bench` is run on a GPU machine and a slow one 🛠 · whether redistributing publishers' full text in the annexes is the operator's to do 🔒 (recorded in the Open queue, not decided).
