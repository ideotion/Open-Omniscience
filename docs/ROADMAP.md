...
- **Self-curating sources — Phase 2 discovery funnel** (ruled 2026-07-13, Q3a/Q4a) — candidate → trial → graduated funnel with trial auto-enable DEFAULT-OFF; the flagship channel = **Wikipedia references across all 12 editions (zero-network)**; funnel states on `Source` (additive migration); **WIRE `external_sources`** as the funnel resolution table (Q4a); an audit view + undo; graduation passes through the Phase-1 quality gate. NOT started (a dedicated backend session; the zero-network wiki-refs channel is the recommended first, most-verifiable slice). ⬜
