...
with sync_playwright() as p:
    ...
    while True:
        ...
        if first_full_shot and reopen_plan and next_action_at is None and s["dt"] > 1.0:
            next_action_at = now
        if first_full_shot and not reopen_plan and not selection_done and s.get("open") and s["dt"] > 1.5 and next_action_at is None:
            try:
        ...
        if selection_done and not sel_checked and sel_info and s["dt"] - sel_info["t"] >= 10:
            ...
            sel_checked = True
        if reopen_plan and next_action_at and now >= next_action_at and api_state == "running":
            act = reopen_plan.pop(0)
            if act.startswith("close"):
                close_import(page); next_action_at = now + 2
                ...
                next_action_at = None; t_sel_ok = True
        # I7: all four Done
        st3 = [x["text"] for x in (s.get("stages") or []) if x["key"] == "search_index"]
        if not all_done_seen and len(rows) == 4 and (all("— Done" in r["text"] for r in rows) or (st3 and "running" in st3[0])):
            all_done_seen = True
            ev("I7_all_done", **s)
    ...
    for i in range(40):
        ...
        if "finishing the re-index" in body:
            ...
            if len([x for x in tm_samples if "finishing the re-index" in x["body"]]) >= 4:
                break
