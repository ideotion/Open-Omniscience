...
with session("R-bulletin") as (page, rec):
    ...
    shot(page, "R6-bulletin-panel-en")
    if rec.obs["controls_visible"]:
        ...
        doc.screenshot(path="/tmp/claude-0/walk/L-recheck/R6-bulletin-doc-en.png", full_page=False)
        with open("/tmp/claude-0/walk/L-recheck/bulletin-doc.txt", "w") as f:
            f.write(text)
        doc.close()
        # undo: Delete
    ...
    print(rec.obs)
import re as _re
_t = open("/tmp/claude-0/walk/L-recheck/bulletin-doc.txt").read()
print("LAW:", _re.findall(r"Law changed[^\n]*", _t)[:2], _re.findall(r"Measured:[^\n]*jurisdiction[^\n]*", _t)[:2])
print("SRCC:", _re.findall(r"Source countries:[^\n]*", _t)[:1], _re.findall(r"^Languages:[^\n]*", _t, _re.M)[:1])
print("LANG-lines:", sorted(set(_re.findall(r"· lang ([a-z]{2,3})\b", _t))), sorted(set(_re.findall(r"detected ([a-z]{2,3})\b", _t))))
