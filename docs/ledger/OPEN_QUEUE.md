# Open queue — pending maintainer rulings

> **Moved verbatim out of `CLAUDE.md` on 2026-09-07** (maintainer ruling A3(1); proposal
> `docs/design/LEDGER_RESTRUCTURE_PROPOSAL_2026-08-04.md` §4.1). **The move was byte-exact**
> for all 165 entries — nothing was summarised, reworded, reordered or dropped, and each
> surviving entry was re-verified as present verbatim afterwards.
>
> **Two changes since the move, both in the same PR.** A3(3) retired two entries whose work
> the tree confirmed shipped — `card-audit.json has not serialised` and `why ten vLLM starts
> died` — to [`SHIPPED_LOG.md`](SHIPPED_LOG.md), verbatim, with rows in
> [`shipped.csv`](shipped.csv). One entry was added per rule (2), recording the eight
> lesson-shaped entries still filed here. **163 of the original 165 remain byte-identical.**
>
> **This is the docket, not the constitution.** Read the entries relevant to the work at
> hand; it is consulted, not memorised (`CLAUDE.md` THE PROTOCOL rule (1)).
>
> **Appending:** a new PENDING ruling, contingency or deliberate-omission note is recorded
> HERE as prose, in the turn it is given (rule (2)). Rule (5) protects every such entry —
> it may never be compressed away. An entry whose work is FULLY shipped leaves for
> [`shipped.csv`](shipped.csv) per rule (5a); an entry still holding any open ruling stays,
...
- **MULTILINGUAL KEYWORD TRANSLATION + SENSE DISAMBIGUATION (maintainer 2026-09-05: "when searching
  ...
  [`docs/design/KEYWORD_TRANSLATION_DISAMBIGUATION_2026-09-05.md`](../design/KEYWORD_TRANSLATION_DISAMBIGUATION_2026-09-05.md);
  ...
  **FOUR CROSS-CHECKS MEASURED AGAINST THE TREE THE SAME DAY (the research pass could not know
  ...
  languages~~ **CORRECTED 2026-09-05 by pass 2 — it covers SEVEN, and the error is the instructive
  part: this cross-check read `_multilingual.yml` ALONE while `_load_extra_stopwords()`
  (`extract.py:397`) unions EVERY `*.yml` in the directory — the sentence this ledger quoted from
  that file's own header at the time ("global_stopwords() unions every file here regardless of
  which one a word lives in"). I quoted the warning and measured as if it did not apply.** Measured
  over the union of all 34 files (2,343 entries): **en 12/12 · fr 12/12 · de 12/12 · es 12/12 · ru
  12/12 in BOTH paradigms · id 9/12 · pt 2/12 · ar 0 · hi 0 · bn 0**; zh/ja have no `一月`-style
  entries and need none. 82 month forms are globally banned, 13 of them for languages we ship no UI
  in (it `maggio`, nl `maart`). **The es/de/ru months are banned from a file named `hi.yml`** —
  headed "Hindi", holding 135 entries of which 39 are LATIN and 24 CYRILLIC: the PR #740/#744
  Phase-4.1 blob→per-file migration put a multilingual month block there. Behaviourally a no-op (the
  union is language-agnostic and a test pins the set byte-identical, exactly as that migration
  promised) but the FILENAME is now false about its contents, and it is what made this measurement
  go wrong — re-filing it into `_multilingual.yml` is a pure set-identical move (slice 3b). So today
  is still the worst of both directions on a WIDER footing: **over-deleting in seven languages** and
  **not filtering datelines at all** in ar/hi/bn (+ pt for ten of twelve). A ru `15 января` IS
  filtered today — the earlier claim otherwise is withdrawn — while an ar `15 يناير` is not. (b) **That makes a
  ...
  lacks** — ru ALL THREE cases tested incl. the prepositional `январе` that CLDR
  OMITS (so for ru the extractor is strictly AHEAD of CLDR — pass 2 recommends a stemmer we do not
  need), ar Gulf/Egyptian AND **part of** Levantine (3 of 7 forms — `كانون الثاني`/`نيسان`/`تموز`/
  `آب` are MISSING; the "AND Levantine" claim was too generous), hi, bn, th, fa, a documented
  ...
  (d) **the ONE real CLDR gap is the Maghrebi system** — WIDENED by pass 2, which surfaced it
  INDEPENDENTLY (as close to corroboration as this exercise gets): the extractor knows **0 of 7** —
  `جانفي`/`فيفري`/`أفريل` plus `ماي`/`جوان`/`جويلية`/`أوت` — a precisely-located live recall gap
  against dz/tn; still a table entry each, now fully enumerated rather than sampled.
  ...
  *disambiguation items* (`P31 = Q4167410`, ~1.4M) exist only to carry interlanguage links; they
  model no concept and list SITELINKS, not senses. This is the structural difference from Wikipedia,
  whose disambiguation PAGE *is* a surface-form→sense index while the Wikidata item representing it
  carries none of them. Anyone building the homonym dictionary reaches for that QID first (this
  ledger already carries it — as a meta-class FILTER for the ring generator) and gets nothing.
  (2) **the mechanism that works is the one this plan already proposed** — two items may carry the
  SAME label in the SAME language (`Curry` = Q2368856 the language and Q5195194 an Alaskan village,
  separated only by the description), so the index is: group labels+aliases by (normalized string,
  language), keep groups ≥2, attach `P31`. That is verbatim the §6 SOURCES line *"harvest the
  AMBIGUITY MAP from the SAME fetch"*, independently arrived at, now with named prior art shipping
  it (OpenTapioca, Apache-2.0) — CC0, no runtime API, and `P31` gives the month/person/org
  distinction AS A STRUCTURED FIELD. (3) **RESOLUTION is NOT shippable and this is the decisive
  negative** — on the one apples-to-apples news comparison (RSS-500, InKB micro F1): best system
  **0.455**, DBpedia Spotlight 0.281, the Wikidata-native non-neural linker **0.335**; non-neural
  WSD sits in a 65-68 F1 band whose FLOOR IS A LOOKUP TABLE (most-frequent-sense), against neural
  in the 80s — all the graph machinery buys under three points. And the numbers are GENEROUS in
  exactly our direction: **the InKB metric everyone reports EXCLUDES out-of-KB mentions**, which is
  precisely the case our requirement makes first-class (of 476 out-of-KB entities in that news set,
  human review recovered 63 — in news, unlinkable is the MAJORITY of out-of-KB). (4) **two coverage
  problems from the people who shipped it** — aliases are curated for the auto-suggest box, not text
  (`Trump`→Q22686 yes, `Cameron`→Q192 NO; misspellings explicitly discouraged), so surname-only
  mentions miss; and **Wikidata carries NO occurrence counts**, killing the cheap prior (the
  published substitute is statement+sitelink counts + Wikidata PageRank, all CC0 from the same dump).
  **THE TREE REFUTES PASS 2's FIRST RECOMMENDATION** (it advises un-banning months PER LANGUAGE,
  first, as "a scoping fix… no accuracy question", claiming it recovers "every one of the losses in
  your table"): (a) **per-language scoping is STRUCTURALLY IMPOSSIBLE here for exactly the two
  languages whose months are banned** — `stopwords_extra/*.yml` is a language-agnostic union BY
  CONSTRUCTION (the loader's own docstring: "NOT a per-language scoping guarantee"), and the real
  scoped channel is unreachable for en/fr because `get_stopwords()` tests `language_stopwords`
  FIRST and the scoped set only in the `elif`, with `LANGUAGE_STOPWORDS` holding exactly `en`+`fr`
  (the recorded stoplist-architecture lesson, decisive here). So it is a stopwords-ARCHITECTURE
  change, not a data file — not wrong, but not free, and it must not be sequenced as though it were.
  (b) **and it recovers 3 of the 7 named losses**: `mars`/`avril`/`mayo` (cross-language) yes;
  `march`/`may`/`april`/`august` still banned because they are ENGLISH months in an English document
  — **the March on Washington is the case pass 2 names as recovered and exactly the case scoping
  cannot touch**. Of 69 banned forms belonging to a UI language, 59 are a month in exactly one and
  10 in several. Scoping and the date-aware drop are COMPLEMENTS, not alternatives; §5's
  recommendation stands and slice 2's diagnostic still decides it. (c) **the unlinked case already
  has a shape in this tree** — pass 2 recommends GERBIL epsilon / TAC KBP NIL; `ArticleMentionedDate`
  already stores `status="candidate"` and filters `status != "rejected"` (`datestore.py:90,233`), a
  human-confirmable three-state candidate model, with the `ai-who`/`ai-place`/`ai-date` kinds as the
  second precedent. NIL GRANULARITY (one per surface string vs one per occurrence) stays open.
  **CORROBORATED BY BOTH PASSES INDEPENDENTLY:** Arabic is NOT one list (split by REGION, not by the
  `ar` tag — scoping by language tag silently fails on lb/sy/iq/dz sources); CLDR is `Unicode-3.0`
  and tiny; and **the month-homonymy problem DOES NOT EXIST in zh/ja** (numeric + 月 cannot collide
  with a name; tree-confirmed 0/12 known, 0 banned) — so a language-agnostic stoplist costs two of
  ...
  **THE ONE OPEN NUMBER in the whole plan** is slice 4's: whether an ambiguous-only 12-language
  ...
  (synthetic random QIDs are the worst case for compression). The leverage is "ambiguous-ONLY":
  unambiguous forms need no sense identity, the existing one-row-per-string model already handles
  them. **OPERATOR STEP — BOTH PASSES DONE (2026-09-05), ONE STEP REMAINS AND IT IS AN ALLOWLIST
...
- **FIELD FEEDBACK 2026-08-07 — governments · law extraction · Feed tab · crash visibility ·
  ...
  record = [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-08-07_FIELD_FEEDBACK.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-08-07_FIELD_FEEDBACK.md);
...
- **FIELD IMPRESSIONS 2026-08-01 — Home-alerts relevance/card system · Home overview subtabs ·
  ...
  **ANSWERS RECEIVED + RULED same day (maintainer answered all 17; briefs of record =
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-08-01_D_HOME_ALERTS_LIBRARY_UI.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-08-01_D_HOME_ALERTS_LIBRARY_UI.md) +
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-08-01_E_AI_COORDINATOR_BENCH.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-08-01_E_AI_COORDINATOR_BENCH.md);
...
- **SETTINGS-TAB REVIEW 2026-07-31 — 15 SUBTABS → 10, A NEW CARDS TAB, A NEW ADVANCED TAB
  ...
  brief of record =
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-31_SETTINGS_RESTRUCTURE.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-31_SETTINGS_RESTRUCTURE.md)
...
- **FIELD REMARKS 2026-07-26 — AI-job toggle UX, translation-gap detector ask, two progressive-sweep
  ...
  brief of record = [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-26_FIELD_REMARKS.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-26_FIELD_REMARKS.md);
...
- **HARDWARE DIAGNOSTICS COMPARISON 2026-07-26 — 7-instance cross-machine analysis (maintainer sent
  ...
  record = [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-26_HARDWARE_DIAGNOSTICS_COMPARISON.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-26_HARDWARE_DIAGNOSTICS_COMPARISON.md);
  ...
  **EXECUTION PLAN AUTHORED same day (maintainer: "optimize it for an autonomous session driven
  ...
  [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-26_EXECUTION_PLAN.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-26_EXECUTION_PLAN.md)
...
- **TRANSVERSAL AUDIT 09 — SECURITY + FUNCTIONAL DELTA (2026-07-25, maintainer-commissioned generic
  ...
  [`docs/audit/09_TRANSVERSAL_AUDIT_0.3_DELTA.md`](../audit/09_TRANSVERSAL_AUDIT_0.3_DELTA.md), a
...
- **FIELD DIAGNOSTICS FINDINGS (2026-07-21, from a real operator export against the live
  ...
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-21_FIELD_DIAGNOSTICS_FINDINGS.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-21_FIELD_DIAGNOSTICS_FINDINGS.md),
...
- **DOC MAP (consolidated 2026-07-10):** the single forward-looking board is now
  [`docs/ROADMAP.md`](../ROADMAP.md) (DB limitations · performance/scale · known bugs ·
  feature backlog, each with status) — read it for the overview; the DEEP scale detail stays
  in [`docs/product/SCALE_ROADMAP.md`](../product/SCALE_ROADMAP.md); design intent stays in
...
- **SCALE MANDATE (maintainer ruled 2026-07-09; the consolidated roadmap lives in
  [`docs/product/SCALE_ROADMAP.md`](../product/SCALE_ROADMAP.md) — read it before picking
...
- **FIELD-TEST 2026-07-08 — full intake + diagnostics action plan captured in
  [`docs/product/field-test-2026-07-08/LEDGER.md`](../product/field-test-2026-07-08/LEDGER.md)
...
- **KEYWORD-ENGINE OPTIMIZATION — STRATEGY + PROGRAM (2026-06-26 research fold-in → the 2026-06-25
  build session; strategy of record = [`docs/design/KEYWORD_ENGINE_OPTIMIZATION_STRATEGY.md`](../design/KEYWORD_ENGINE_OPTIMIZATION_STRATEGY.md),
...
- **FIX SESSION 2026-07-14 — data-safety + field-diagnostic + law + Tails (plan of record =
  [`docs/design/FIX_SESSION_PROMPT_2026-07-14.md`](../design/FIX_SESSION_PROMPT_2026-07-14.md), #666,
...
- **V1 PATHWAY — PLANNING SESSION 2026-07-14 (maintainer-directed, docs-only; plan of record =
  [`docs/design/V1_PATHWAY_2026-07-14.md`](../design/V1_PATHWAY_2026-07-14.md); draft PR onto 0.2,
  ...
  **ADDENDUM 3 — OPTIMIZATION-TAIL SESSION BRIEF (maintainer-asked 2026-07-14, same session):**
  ...
  left open = [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-14_OPTIMIZATION_TAIL.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-14_OPTIMIZATION_TAIL.md)
...
- **DOCUMENTATION REVIEW — SURVEY + ACTION PLAN (maintainer-asked 2026-07-17, docs-only; plan of
  record = [`docs/design/ACTION_PLAN_2026-07-17_DOCS_REVIEW.md`](../design/ACTION_PLAN_2026-07-17_DOCS_REVIEW.md),
...
- **LAW VERTICAL — INVESTIGATION + SESSION BRIEF (maintainer-asked 2026-07-17: "a proper,
  ...
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-17_LAW_VERTICAL.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-17_LAW_VERTICAL.md)):**
  ...
  **ACQUISITION CHANNEL RULED + INTAKE SHIPPED same day (maintainer: "a parallel, autonomous,
  ...
  [`docs/design/LAW_SOURCES_ACQUISITION_2026-07-17.md`](../design/LAW_SOURCES_ACQUISITION_2026-07-17.md)
...
- **LEMMATIZATION DEFAULT-ON — MAINTAINER RULED 2026-07-18 (the measure-gate is SATISFIED;
  brief of record =
  [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_LEMMA_DEFAULT_ON.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_LEMMA_DEFAULT_ON.md);
...
- **ENTITY FAMILIES AT REAL SCALE — FIELD EXPORT + SESSION BRIEF (maintainer 2026-07-18, the
  ...
  [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_FAMILIES_ENTITIES.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_FAMILIES_ENTITIES.md);
...
- **SUPER-GROUPS: HONEST STATS + A LEADS FAMILY + NAVIGATION — FIELD EXPORT + SESSION BRIEF
  ...
  **AMENDED same day (maintainer rulings after the ring-country-map review — brief =
  [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_GROUPS_LAYER_AMENDMENT.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_GROUPS_LAYER_AMENDMENT.md),
...
- **LEADS/CARD-SYSTEM CALIBRATION AT REAL SCALE — FIELD EXPORT + SESSION BRIEF (maintainer
  ...
  [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_LEADS_CALIBRATION.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_LEADS_CALIBRATION.md)):**
  ...
  **AMENDED same day (maintainer field export of Insights→Convergence, default 7-day window, on
  ...
  [`docs/archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_CONVERGENCE_AMENDMENT.md`](../archive/session-briefs/AUTONOMOUS_SESSION_BRIEF_2026-07-18_CONVERGENCE_AMENDMENT.md),
...
- **THE OBSERVATORY — THE CORPUS AS A NIGHT SKY, A DEDICATED TAB (maintainer ruled 2026-07-18 —
  ...
  design of record = [`docs/design/OBSERVATORY_DESIGN.md`](../design/OBSERVATORY_DESIGN.md);
...
- **LLM SOURCE-TAG ASSIGNMENT FROM TOP KEYWORDS (maintainer proposed 2026-07-20: "a source
  ...
  **GO RULED same day (maintainer: run the §8 triage AND the source-tag assignment on the
  ...
  brief of record =
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-20_LLM_TRIAGE_TAG_RUNS.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-20_LLM_TRIAGE_TAG_RUNS.md)
...
- **THE 0.3 CLOSE GATE (maintainer RULED 2026-07-20 — the conditions for tagging v0.3.0;
  ...
  **THE CHECKABLE INVENTORY NOW EXISTS (2026-08-13):
  [`docs/product/RELEASE_0.3_GATE.md`](../product/RELEASE_0.3_GATE.md) is the tickable
  ...
  [`RELEASE_0.2_GATE.md`](../product/RELEASE_0.2_GATE.md) was reconstructed alongside it
...
- **DESIGN-FOLDER AUDIT + REMEDIATION PLAN (2026-07-22, user-requested; full detail
  in [`docs/design/ACTION_PLAN_2026-07-22_DESIGN_AUDIT_REMEDIATION.md`](../design/ACTION_PLAN_2026-07-22_DESIGN_AUDIT_REMEDIATION.md)):**
...
- **GUI AUDIT 2026-07-28 — TRANSLATION COVERAGE · GRAPHICAL QUALITY · VISUAL DATA REPRESENTATION
  ...
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-28_GUI_AUDIT.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-28_GUI_AUDIT.md);
...
- **SYSTEMATIC GUI TEST & CRITICAL REVIEW — EXECUTED 2026-07-22 (maintainer-asked; brief of record =
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-22_GUI_SYSTEMATIC_TEST.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-22_GUI_SYSTEMATIC_TEST.md);
  report = [`docs/audit/GUI_TEST_REPORT_2026-07-22.md`](../audit/GUI_TEST_REPORT_2026-07-22.md);
...
- **PR #740 + PR #744 REMEDIATION — SESSION BRIEF (maintainer-asked 2026-07-22, "have a careful
  ...
  record = [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-22_PR740_PR744_REMEDIATION.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-22_PR740_PR744_REMEDIATION.md);
...
- **FIELD FEEDBACK 2026-07-23 — seven impressions from multi-VM/multi-machine use (maintainer;
  brief of record = [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-23_FIELD_FEEDBACK_WORKFLOW.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-23_FIELD_FEEDBACK_WORKFLOW.md),
...
- **FIELD FEEDBACK 2026-07-24 — eight impressions (language detection · Governments/law ·
  ...
  **ANSWERS RECEIVED + RULED same day (maintainer answered all 16 questions; briefs of record =
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-24_A_FIELD_FIXES.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-24_A_FIELD_FIXES.md)
  + [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-24_B_AI_STACK.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-24_B_AI_STACK.md);
...
- **SCRAPING/DOWNLOAD 10× SCALING — FIVE TACTICAL STRATEGIES (maintainer-asked 2026-07-24,
  ...
  **EXTENDED same day (maintainer: "add all of them to the PR in a coherent fashion so the coding
  ...
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-07-24_C_THROUGHPUT_SCALING.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-07-24_C_THROUGHPUT_SCALING.md)
...
- **SYSTEMATIC CRASHES ACROSS FOUR MACHINES — FIELD ANALYSIS + EIGHT RULINGS (maintainer
  ...
  [`docs/design/AUTONOMOUS_SESSION_BRIEF_2026-09-02_CRASH_ROOT_CAUSE.md`](../design/AUTONOMOUS_SESSION_BRIEF_2026-09-02_CRASH_ROOT_CAUSE.md);
...
- **WHOLE-REPOSITORY ANALYSIS + THE 23-PROMPT ACTION PLAN (maintainer-asked 2026-09-06: "have a detailed
  ...
  plan of record = [`docs/plans/2026-09-06-repo-analysis/`](../plans/2026-09-06-repo-analysis/),
...
- **EIGHT LESSON-SHAPED ENTRIES ARE MISFILED IN THIS QUEUE — a ruling is owed on relocating
  ...
  each states a defect, its mechanism and an explicit **GENERAL FORM** — sitting in the docket
  rather than in `LESSONS.md`, which is where protocol rule (5a)(b) puts a lesson and where
  rule (1) makes it mandatory reading. In queue order they are: *A MODEL THAT SILENTLY RETURNS
  THE SOURCE UNCHANGED* · *A MEASUREMENT IS ONLY OVER THE ROWS THAT PRODUCED DATA* · *A
  CONTAINMENT GUARD WRITTEN AS A STRING PREFIX* · *BOTH i18n RATCHETS ARE BLIND TO A STRING IN
  AN OBJECT LITERAL* · *AN ADAPTIVE CONTROLLER REBUILT AT THE TOP OF EVERY LOOP IS AMNESIAC* ·
  *A MODE-SPECIFIC DEFAULT THAT OVERWRITES AN EXPLICIT ARGUMENT* · *NARROWING A BOUND MAKES
  EVERY COMPARISON AGAINST THE OLD BOUND UNREACHABLE* · *A "NO OPINION" RETURN EXPRESSED AS A
  CONCRETE DEFAULT*. Every fix they describe was VERIFIED SHIPPED this pass
  (`uninstall.py:106` `is_relative_to`, `bandwidth.py:122` `ramp_ceiling`, `capacity.py:116`
  `seed_for -> int | None`).
  **WHY THEY WERE NOT MOVED, and it is not timidity:** the A3(3) ruling authorises retiring
  only entries verified STANDALONE, and these are not. *A MODEL THAT SILENTLY RETURNS…* opens
  "from the same field probe" — that probe is described in the entry ABOVE it, which is the
  live ONE-MODEL ruling; *A MODE-SPECIFIC DEFAULT…* says "the fix above"; *A "NO OPINION"
  RETURN…* says "the exact mirror of the entry above it". Relocating them therefore means
  deciding how to preserve a chain of cross-references into an entry that must stay — a
  ledger-structure judgement of exactly the class A3 itself was, and one this session was not
  authorised to make. Moving six and leaving two would also split a coherent run.
  **THE COST OF LEAVING THEM, stated plainly:** the A3(2) amendment makes the queue
  consulted-on-demand, so these eight moved from mandatory reading to optional — a demotion
  this restructure introduced. Mitigated, not fixed, by a pointer at the head of `LESSONS.md`
  naming them.
  **THE OPTIONS:** (a) move the contiguous run *A CONTAINMENT GUARD* → *A "NO OPINION" RETURN*
  (six entries, all internal cross-references preserved) as one verbatim block, and leave the
  two that reference the ONE-MODEL entry where they are; (b) move all eight and add one
  sentence to each of the three that reference a neighbour, naming the entry instead of its
